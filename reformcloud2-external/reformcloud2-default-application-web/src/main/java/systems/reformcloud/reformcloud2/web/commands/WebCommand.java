...
public class WebCommand extends GlobalCommand {
  ...
  public WebCommand() {
    super("web", "reformcloud.command.web", "The default web command",
          new ArrayList<>());
  }

  public boolean tryAwait(long time, TokenWebServerAuth auth,
  ...
  public boolean handleCommand(@Nonnull CommandSource commandSource,
    ...
    if (strings.length == 1 && strings[0].equalsIgnoreCase("verify")) {
      ...
      if (waiting.getFirst() + TimeUnit.SECONDS.toMillis(15) <=
          System.currentTimeMillis()) {
        return true;
      }

      Objects.requireNonNull(waiting.getSecond())
          .complete(waiting.getFirst(), waiting.getThird());
    }
