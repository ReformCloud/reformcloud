...
public class NukkitSignSystemAdapter
    implements SignSystemAdapter<BlockEntitySign> {
  ...
  public NukkitSignSystemAdapter(PluginBase plugin, SignConfig signConfig) {
    ...
    ExecutorAPI.getInstance().getEventManager().registerListener(
        new CloudListener());
    Server.getInstance().getPluginManager().registerEvents(new NukkitListener(),
                                                           plugin);
  ...
  @Nonnull
  @Override
  public CloudSign createSign(@Nonnull BlockEntitySign blockEntitySign,
  ...
  private void clearSign(BlockEntitySign sign) {
    ...
    sign.setText(); // Nukkit sets an empty line if the array does not contain a
                    // string for a line
  }
  ...
  private void updateSign(CloudSign sign, SignSubLayout layout,
    ...
    if (nukkit == null || layout.getLines() == null ||
        layout.getLines().length != 4) {
      return;
    }

    String[] lines =
        Arrays.stream(layout.getLines())
            .map(e -> replaceAll(e, sign.getGroup(), processInformation))
            .toArray(String[] ::new);
  ...
  private void changeBlockBehind(BlockEntitySign sign, SignSubLayout layout) {
    ...
    Location location =
        post.getSide(post.getBlockFace().getOpposite()).getLocation();
  ...
  private void runTasks() {
    taskID = Server.getInstance()
                 .getScheduler()
                 .scheduleRepeatingTask(
                     plugin, this::updateAllSigns,
                     fromLong(20 * config.getUpdateInterval()), true)
                 .getTaskId();
  }
  ...
  public static NukkitSignSystemAdapter getInstance() { return instance; }
}
