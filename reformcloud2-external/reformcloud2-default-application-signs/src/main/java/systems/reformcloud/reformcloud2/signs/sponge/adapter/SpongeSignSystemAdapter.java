...
public class SpongeSignSystemAdapter implements SignSystemAdapter<Sign> {
  ...
  static {
    Arrays.stream(BlockTypes.class.getDeclaredFields())
        .filter(e
  ...
  public SpongeSignSystemAdapter(Object plugin, SignConfig config) {
    ...
    CommandSpec signs =
        CommandSpec.builder()
            .description(
                Text.of("The default signs command of the cloud system"))
            .permission("reformcloud.command.signs")
            .arguments(GenericArguments.optional(
                           GenericArguments.string(Text.of("Execute type"))),
                       GenericArguments.optional(
                           GenericArguments.string(Text.of("Target group"))))
  ...
  private void updateSign(CloudSign sign, SignSubLayout layout,
    ...
    if (sponge == null || layout.getLines() == null ||
        layout.getLines().length != 4) {
      return;
    }

    SignData signData = sponge.getSignData();
    signData.setElement(0, replaceAll(layout.getLines()[0], sign.getGroup(),
                                      processInformation));
    signData.setElement(1, replaceAll(layout.getLines()[1], sign.getGroup(),
                                      processInformation));
    signData.setElement(2, replaceAll(layout.getLines()[2], sign.getGroup(),
                                      processInformation));
    signData.setElement(3, replaceAll(layout.getLines()[3], sign.getGroup(),
                                      processInformation));
  ...
  private void changeBlockBehind(Sign sign, SignSubLayout layout) {
    Optional<DirectionalData> directionalData =
        sign.getLocation().get(DirectionalData.class);
    ...
    data.get(Keys.DIRECTION).ifPresent(e -> {
      Location<World> blockRelative =
          sign.getLocation().getBlockRelative(e.getOpposite());
  ...
  private Text replaceAll(String line, String group,
                          ProcessInformation processInformation) {
    ...
    return PlaceHolderUtil.format(line, group, processInformation,
                                  TextSerializers.FORMATTING_CODE::deserialize);
  }

  private void start() {
    systems.reformcloud.reformcloud2.executor.api.common.utility.task.Task
        .EXECUTOR.execute(() -> {
  ...
  private void runTasks() {
    this.taskID =
        repeat(this::updateAllSigns, this.config.getUpdateInterval() * 20);
  }
  ...
  public static SpongeSignSystemAdapter getInstance() { return instance; }
}
