...
public class NukkitCommandSigns implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender commandSender, Command command,
    ...
    if (!(commandSender.hasPermission("reformcloud.command.signs")) ||
    ...
    if (strings.length == 2 && strings[0].equalsIgnoreCase("create")) {
      if (ExecutorAPI.getInstance()
              .getSyncAPI()
              .getGroupSyncAPI()
              .getProcessGroup(strings[1]) == null) {
        commandSender.sendMessage("§7The process group " + strings[1] +
                                  " does not exists");
        return true;
      }
      ...
      if (block == null ||
          !(block.getLevel().getBlockEntity(block.getLocation()) instanceof
            BlockEntitySign)) {
        commandSender.sendMessage("§cThe target Block is not a sign");
        return true;
      }

      BlockEntitySign entitySign =
          (BlockEntitySign)block.getLevel().getBlockEntity(block.getLocation());
      CloudSign cloudSign = NukkitSignSystemAdapter.getInstance().getSignAt(
          NukkitSignSystemAdapter.getInstance().getSignConverter().to(
              entitySign));
      if (cloudSign != null) {
      ...
      commandSender.sendMessage(
          "§7Created the sign successfully, please wait a second...");
      return true;
    }
