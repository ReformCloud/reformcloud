...
public class SpongeCommandSigns implements CommandExecutor {
  ...
  public CommandResult execute(@Nonnull CommandSource src,
    ...
    if (type != null && target != null && type.equalsIgnoreCase("create")) {
      if (ExecutorAPI.getInstance()
              .getSyncAPI()
              .getGroupSyncAPI()
              .getProcessGroup(target) == null) {
        src.sendMessage(
            Text.of("§7The process group " + target + " does not exists"));
        return CommandResult.success();
      }

      Optional<BlockRayHit<World>> end = getSign(player);
      if (!end.isPresent() ||
          !(end.get().getLocation().getTileEntity().orElse(null) instanceof
            Sign)) {
        src.sendMessage(Text.of(
            "§cThe target Block is not a sign, try step one block back"));
        return CommandResult.success();
      }
      ...
      src.sendMessage(
          Text.of("§7Created the sign successfully, please wait a second..."));
      return CommandResult.success();
    }
    ...
    if (type != null && type.equalsIgnoreCase("delete")) {
      ...
      src.sendMessage(
          Text.of("§7Deleted the sign successfully, please wait a second..."));
      return CommandResult.success();
    }
  ...
  private Optional<BlockRayHit<World>> getSign(Player player) {
    return BlockRay.from(player)
        .distanceLimit(15)
        .narrowPhase(true)
        .stopFilter(lastHit -> {
          final BlockType blockType = lastHit.getExtent().getBlockType(
              lastHit.getBlockX(), lastHit.getBlockY(), lastHit.getBlockZ());
          return blockType.equals(BlockTypes.STANDING_SIGN) ||
