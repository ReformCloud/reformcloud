...
public interface SignSystemAdapter<T> {
  ...
  static SignSystemAdapter getInstance() { return instance.get(); }
   ...
   */
  @Nonnull CloudSign createSign(@Nonnull T t, @Nonnull String group);
   ...
   * @return The sign at the location or {@code null} if the sign does not
   *     exists
   */
  @Nullable CloudSign getSignAt(@Nonnull CloudLocation location);

  /**
   * @return The converter for all objects from the implementation to the cloud
   */
  @Nonnull SignConverter<T> getSignConverter();
