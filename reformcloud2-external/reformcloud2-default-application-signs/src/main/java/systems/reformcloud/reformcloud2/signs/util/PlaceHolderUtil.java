...
public class PlaceHolderUtil {
  ...
  private PlaceHolderUtil() { throw new UnsupportedOperationException(); }

  public static <T> T format(String line, String group,
    ...
    line = line.replace("%uniqueid%",
                        processInformation.getProcessUniqueID().toString());
    line = line.replace("%state%", processInformation.getProcessState().name());
    line = line.replace(
        "%connected%",
        Boolean.toString(processInformation.getNetworkInfo().isConnected()));
    line =
        line.replace("%template%", processInformation.getTemplate().getName());
    line = line.replace("%online%",
                        Integer.toString(processInformation.getOnlineCount()));
    line = line.replace("%max%",
                        Integer.toString(processInformation.getMaxPlayers()));
    line = line.replace("%whitelist%",
                        Boolean.toString(processInformation.getProcessGroup()
