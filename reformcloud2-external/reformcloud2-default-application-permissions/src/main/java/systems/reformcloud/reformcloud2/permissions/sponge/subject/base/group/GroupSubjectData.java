...
public class GroupSubjectData extends AbstractSpongeSubjectData {
  ...
  public GroupSubjectData(@Nonnull String group) { this.group = group; }
  ...
  private Map<String, Boolean> getPermissions() {
    ...
    PermissionGroup permissionGroup =
        PermissionAPI.getInstance().getPermissionUtil().getGroup(group);
    ...
    permissionGroup.getSubGroups().forEach(e -> {
      PermissionGroup sub =
          PermissionAPI.getInstance().getPermissionUtil().getGroup(e);
      if (sub == null) {
        return;
      }
