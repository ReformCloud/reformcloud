...
public class UUIDFetcher {
  ...
  private UUIDFetcher() { throw new UnsupportedOperationException(); }
  ...
  public static UUID getUUIDFromName(final String name) {
    ...
    try {
      HttpURLConnection httpURLConnection =
          (HttpURLConnection) new URL("https://api.minetools.eu/uuid/" + name)
              .openConnection();
      ...
      try (InputStreamReader reader =
        ...
        if (configuration.has("id")) {
          try {
            UUID uuid =
                fromString(PATTERN.matcher(configuration.getString("id"))
  ...
  private static UUID fromDatabase(String name) {
    JsonConfiguration configuration =
        ExecutorAPI.getInstance().getSyncAPI().getDatabaseSyncAPI().find(
            DefaultPermissionUtil.PERMISSION_NAME_TO_UNIQUE_ID_TABLE, name,
            null);
    return configuration == null || !configuration.has("id")
        ? null
        : configuration.get("id", UUID.class);
  }
