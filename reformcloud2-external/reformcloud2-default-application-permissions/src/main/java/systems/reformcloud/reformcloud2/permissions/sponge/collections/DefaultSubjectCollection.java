...
public abstract class DefaultSubjectCollection implements SubjectCollection {
  ...
  @Nonnull protected abstract Subject load(String id);
  ...
  newSubjectReference(@Nonnull String subjectIdentifier) {
  ...
  @Override
  @Nonnull
  public CompletableFuture<Map<String, Subject>>
  loadSubjects(@Nonnull Set<String> identifiers) {
  ...
  @Override
  @Nonnull
  public Map<Subject, Boolean>
  getLoadedWithPermission(@Nonnull String permission) {
  ...
  @Nonnull
  public Map<Subject, Boolean>
  getLoadedWithPermission(@Nullable Set<Context> contexts,
                          @Nonnull String permission) {
    Map<Subject, Boolean> out = new ConcurrentHashMap<>();
    getLoadedSubjects().forEach(e -> {
      Tristate tristate = e.getPermissionValue(
          contexts == null ? e.getActiveContexts() : contexts, permission);
  ...
  @Override
  @Nonnull
  public CompletableFuture<Map<SubjectReference, Boolean>>
  getAllWithPermission(@Nonnull String permission) {
    return CompletableFuture.completedFuture(
        getLoadedWithPermission(permission)
            .entrySet()
            .stream()
            .collect(Collectors.toMap(
                e -> e.getKey().asSubjectReference(), Map.Entry::getValue)));
  }

  @Override
  @Nonnull
  public CompletableFuture<Map<SubjectReference, Boolean>>
  getAllWithPermission(@Nonnull Set<Context> contexts,
                       @Nonnull String permission) {
    return CompletableFuture.completedFuture(
        getLoadedWithPermission(contexts, permission)
            .entrySet()
            .stream()
            .collect(Collectors.toMap(
                e -> e.getKey().asSubjectReference(), Map.Entry::getValue)));
  ...
  public CompletableFuture<Set<String>> getAllIdentifiers() {
    return CompletableFuture.completedFuture(getLoadedSubjects()
                                                 .stream()
