...
public class GeneralCheck {
  ...
  public static boolean hasPermission(PermissionGroup permissionGroup,
                                      String perm) {
  ...
  private static boolean has(PermissionGroup permissionGroup, String perm) {
    if (permissionGroup.getPermissionNodes().stream().anyMatch(
            e -> e.getActualPermission().equals(perm) && e.isSet())) {
      return true;
    }
