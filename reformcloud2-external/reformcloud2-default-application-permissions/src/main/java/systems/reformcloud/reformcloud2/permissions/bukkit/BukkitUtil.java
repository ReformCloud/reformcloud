...
public class BukkitUtil {
  ...
  private BukkitUtil() { throw new UnsupportedOperationException(); }
      ...
      try {
        ...
        String version =
            Bukkit.getServer().getClass().getPackage().getName().split(
                "\\.")[3];
        PERM_FIELD = Class
                         .forName("org.bukkit.craftbukkit." + version +
                                  ".entity.CraftHumanEntity")
                         .getDeclaredField("perm");
        PERM_FIELD.setAccessible(true);
      } catch (final Throwable ex) {
        // glowstone
        PERM_FIELD = Class.forName("net.glowstone.entity.GlowHumanEntity")
                         .getDeclaredField("permissions");
        PERM_FIELD.setAccessible(true);
      }
    } catch (final ClassNotFoundException | NoSuchFieldException ex) {
      throw new RuntimeException(
          "Error while obtaining bukkit or glowstone perm fields (are you using your own build?)",
          ex);
    }
  }
