...
public class CommandLeave implements Command {
  ...
  public CommandLeave(@Nonnull List<String> aliases) { this.aliases = aliases; }
  ...
  public void execute(CommandSource commandSource, @NonNull String[] strings) {
    ...
    if (lobby != null) {
      player.sendMessage(
          TextComponent.of(VelocityExecutor.getInstance().getMessages().format(
              VelocityExecutor.getInstance().getMessages().getConnectingToHub(),
              lobby.getName())));
      player
          .createConnectionRequest(VelocityExecutor.getInstance()
                                       .getProxyServer()
