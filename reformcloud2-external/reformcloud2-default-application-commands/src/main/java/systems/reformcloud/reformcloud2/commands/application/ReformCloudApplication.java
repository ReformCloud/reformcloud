...
public class ReformCloudApplication extends Application {
  ...
  public void onEnable() {
    ...
    commandsConfig = JsonConfiguration.read(path).get(
        "config", new TypeToken<CommandsConfig>() {});
  ...
  public void onPreDisable() {
    commandsConfig =
        new CommandsConfig(false, new ArrayList<>(), false, new ArrayList<>());
    ...
    ExecutorAPI.getInstance()
        ...
        .forEach(e
                 -> DefaultChannelManager.INSTANCE.get(e.getName())
                        .ifPresent(s
                                   -> s.sendPacket(
                                       new PacketOutRegisterCommandsConfig())));
  }

  public static CommandsConfig getCommandsConfig() { return commandsConfig; }
}
