...
public final class Runner {
  ...
  static {
    System.setProperty("reformcloud.runner.version",
                       Runner.class.getPackage().getImplementationVersion());

    System.setProperty("reformcloud.runner.specification",
                       Runner.class.getPackage().getSpecificationVersion());
  }

  private static final Predicate<String> CONTROLLER_UNPACK_TEST = s
      -> s != null &&
             (s.equalsIgnoreCase("controller") ||
              s.equalsIgnoreCase("client") || s.equalsIgnoreCase("node"));

  private static final Supplier<String> CHOOSE_INSTALL_MESSAGE =
      () -> "Please choose an executor: [\"controller\", \"client\", \"node\"]";
  ...
  public static synchronized void main(String[] args) {
    if (!isAPI()) {
      startSetup((version, id) -> {
        ...
        ClassLoader classLoader = ClassPreparer.create(file.toPath(), path -> {
          URL[] urls = new URL[] {path.toUri().toURL()};
          return new RunnerClassLoader(urls);
        });
        if (!(classLoader instanceof URLClassLoader)) {
          throw new RuntimeException(
              "ClassLoader has to be an url class loader");
        }
        ...
        try (JarFile jarFile = new JarFile(file)) {
          ...
          Method invoke =
              classLoader.loadClass(main).getMethod("main", String[].class);

          if (id.equals("1") || id.equals("2") || id.equals("4")) {
            createInvoke(id);
          } else {
            throw new RuntimeException(
                "Unknown id provided by config detected");
          }
      ...
      runIfProcessExists(path -> {
        ...
        if (!(classLoader instanceof URLClassLoader)) {
          throw new RuntimeException(
              "ClassLoader has to be a url class loader");
        }
  ...
  private static void startSetup(BiConsumer<String, String> andThen) {
    ...
    if (Files.exists(Paths.get("reformcloud/.bin/config.properties"))) {
      try (InputStream inputStream = Files.newInputStream(
               Paths.get("reformcloud/.bin/config.properties"))) {
        properties.load(inputStream);
      } catch (final IOException ex) {
        ex.printStackTrace();
      }

      andThen.accept(properties.getProperty("reformcloud.version"),
  ...
  private static void unpackExecutor() {
    try (InputStream inputStream =
      ...
      Files.copy(Objects.requireNonNull(inputStream),
                 Paths.get("reformcloud/.bin/executor.jar"),
  ...
  private static void unpackExecutorAsPlugin() {
    try (InputStream inputStream =
             Runner.class.getClassLoader().getResourceAsStream(
                 "files/executor.jar")) {
      Files.copy(Objects.requireNonNull(inputStream),
                 Paths.get("plugins/executor.jar"),
                 StandardCopyOption.REPLACE_EXISTING);
    } catch (final Exception ex) {
      ex.printStackTrace();
    }
  ...
  private static void invokeMethod(Method method, Object args,
  ...
  public static void premain(String string, Instrumentation inst) {
    if (System.getProperty("reformcloud.lib.path") == null ||
        System.getProperty("reformcloud.process.path") == null) {
      return;
    }

    String libPath =
        System.getProperty("reformcloud.lib.path") + "/reformcloud/.bin/libs/";
    ...
    try {
      inst.appendToSystemClassLoaderSearch(new JarFile(
          new File(System.getProperty("reformcloud.process.path"))));
