...
public class GroupAPIImplementation implements GroupAsyncAPI, GroupSyncAPI {
  ...
  private final ControllerExecutorConfig controllerExecutorConfig =
      ControllerExecutor.getInstance().getControllerExecutorConfig();
  ...
  createProcessGroupAsync(@Nonnull ProcessGroup processGroup) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(
                controllerExecutorConfig.createProcessGroup(processGroup)));
    return task;
  }
  ...
  public Task<MainGroup> getMainGroupAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(
                Links.filter(controllerExecutorConfig.getMainGroups(),
                             mainGroup -> mainGroup.getName().equals(name))));
    return task;
  }
  ...
  public Task<ProcessGroup> getProcessGroupAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(Links.filter(
                controllerExecutorConfig.getProcessGroups(),
                processGroup -> processGroup.getName().equals(name))));
    return task;
  }
  ...
  public Task<Void> deleteMainGroupAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(() -> {
      Links
          .filterToReference(controllerExecutorConfig.getMainGroups(),
                             mainGroup -> mainGroup.getName().equals(name))
          .ifPresent(controllerExecutorConfig::deleteMainGroup);
      task.complete(null);
    });
    return task;
  }
    ...
    Task.EXECUTOR.execute(() -> {
      Links
          .filterToReference(
              controllerExecutorConfig.getProcessGroups(),
              processGroup -> processGroup.getName().equals(name))
          .ifPresent(controllerExecutorConfig::deleteProcessGroup);
      task.complete(null);
    });
    return task;
  }
  ...
  public Task<List<MainGroup>> getMainGroupsAsync() {
    ...
    Task.EXECUTOR.execute(()
                              -> task.complete(Collections.unmodifiableList(
                                  controllerExecutorConfig.getMainGroups())));
    return task;
  }
