...
public final class ControllerExecutorConfig {
  ...
  private static final Collection<Path> PATHS =
      newCollection(s
                    -> Paths.get(s),
                    "reformcloud/groups/main", "reformcloud/groups/sub",
                    "reformcloud/configs", "reformcloud/applications");
  ...
  public ControllerExecutorConfig() {
    createDirectories();
    this.subGroupRegistry =
        RegistryBuilder.newRegistry(Paths.get("reformcloud/groups/sub"));
    this.mainGroupRegistry =
        RegistryBuilder.newRegistry(Paths.get("reformcloud/groups/main"));
    ...
    this.ingameMessages =
        JsonConfiguration.read("reformcloud/configs/messages.json")
            .get("messages", IngameMessages.TYPE);
  }

  private ControllerConfig load() {
    return JsonConfiguration.read(ControllerConfig.PATH)
        .get("config", ControllerConfig.TYPE);
  }

  private String connectionKey() {
    return JsonConfiguration.read("reformcloud/.bin/connection.json")
        .getString("key");
  }

  private void loadGroups() {
    processGroups.addAll(
        this.subGroupRegistry.readKeys(e -> e.get("key", ProcessGroup.TYPE)));
    mainGroups.addAll(
        this.mainGroupRegistry.readKeys(e -> e.get("key", MainGroup.TYPE)));
  }
  ...
  private void firstSetup() {
    new JsonConfiguration()
        .add("key", StringUtil.generateString(50))
        .write(Paths.get("reformcloud/.bin/connection.json"));
    new JsonConfiguration()
        .add("messages", new IngameMessages())
        .write(Paths.get("reformcloud/configs/messages.json"));
    ...
    setup
                   ...
                   .add("config",
                        new ControllerConfig(
                            ...
                            Collections.singletonList(
                                Collections.singletonMap(s.trim(), 2008)),
                            Collections.singletonList(
                                Collections.singletonMap(s.trim(), 1809))))
        ...
        .addQuestion(new DefaultSetupQuestion(
            ...
            s -> {
              MainGroup mainProxy = new DefaultMainGroup(
                  "Proxy", Collections.singletonList("Proxy"));
              MainGroup mainLobby = new DefaultMainGroup(
                  "Lobby", Collections.singletonList("Lobby"));
              ...
              case 1: {
                proxy = new DefaultProcessGroup(
                    "Proxy", 25565, Version.BUNGEECORD, 128, true, 512);
                lobby = new DefaultProcessGroup(
                    "Lobby", 41000, Version.SPIGOT_1_15, 512, false, 50);
                break;
              }

              case 2: {
                proxy = new DefaultProcessGroup(
                    "Proxy", 19132, Version.WATERDOG, 128, true, 512);
                lobby = new DefaultProcessGroup(
                    "Lobby", 41000, Version.NUKKIT_X, 512, false, 50);
                break;
              }
              ...
              if (proxy == null) {
                throw new IllegalStateException(
                    "Lobby or Proxy group not initialized correctly");
              }
  ...
  public MainGroup createMainGroup(MainGroup mainGroup) {
    MainGroup mainGroup1 =
        mainGroups.stream()
            .filter(group -> mainGroup.getName().equals(group.getName()))
            .findFirst()
            .orElse(null);
    if (mainGroup1 == null) {
  ...
  public void updateProcessGroup(ProcessGroup processGroup) {
    Links
        .filterToReference(
            processGroups,
            group -> processGroup.getName().equals(group.getName()))
        .ifPresent(group -> {
  ...
  public void updateMainGroup(MainGroup mainGroup) {
    Links
        .filterToReference(mainGroups,
                           group -> group.getName().equals(mainGroup.getName()))
        .ifPresent(group -> {
  ...
  public List<MainGroup> getMainGroups() { return new ArrayList<>(mainGroups); }
  ...
  public IngameMessages getIngameMessages() { return ingameMessages; }

  public final boolean isFirstStartup() { return firstStartup.get(); }
}
