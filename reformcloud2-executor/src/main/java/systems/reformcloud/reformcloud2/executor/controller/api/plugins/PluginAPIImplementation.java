...
public class PluginAPIImplementation implements PluginSyncAPI, PluginAsyncAPI {
  ...
  public Task<Void> installPluginAsync(@Nonnull String process,
    ...
    Task.EXECUTOR.execute(() -> {
      DefaultChannelManager.INSTANCE.get(process).ifPresent(
          packetSender
          -> packetSender.sendPacket(new ControllerPluginAction(
              ControllerPluginAction.Action.INSTALL,
              new DefaultInstallablePlugin(plugin.getDownloadURL(),
    ...
    Task.EXECUTOR.execute(() -> {
      DefaultChannelManager.INSTANCE.get(process).ifPresent(
          ...
          -> packetSender.sendPacket(new ControllerPluginAction(
              ...
              new DefaultPlugin(plugin.version(), plugin.author(),
                                plugin.main(), plugin.depends(),
                                plugin.softpends(), plugin.enabled(),
                                plugin.getName()))));
      task.complete(null);
    });
    return task;
    ...
    Task.EXECUTOR.execute(() -> {
      ...
      task.complete(
          Links.filter(processInformation.getPlugins(),
                       defaultPlugin -> defaultPlugin.getName().equals(name)));
    });
    return task;
  }
  ...
  getPluginsAsync(@Nonnull String process, @Nonnull String author) {
    ...
    Task.EXECUTOR.execute(() -> {
      ...
      task.complete(Links.allOf(processInformation.getPlugins(),
                                defaultPlugin
                                -> defaultPlugin.author() != null &&
                                       defaultPlugin.author().equals(author)));
    });
    return task;
  }
  ...
  public void installPlugin(@Nonnull String process,
                            @Nonnull InstallablePlugin plugin) {
    installPluginAsync(process, plugin)
        .awaitUninterruptedly(TimeUnit.SECONDS, 5);
  }
  ...
  public void unloadPlugin(@Nonnull String process, @Nonnull Plugin plugin) {
    unloadPluginAsync(process, plugin)
        .awaitUninterruptedly(TimeUnit.SECONDS, 5);
  }
  ...
  public Plugin getInstalledPlugin(@Nonnull String process,
                                   @Nonnull String name) {
    return getInstalledPluginAsync(process, name)
