...
public class PacketInSyncProcessGroups implements NetworkHandler {
  ...
  public void handlePacket(PacketSender packetSender, Packet packet,
    ...
    Collection<ProcessGroup> groups = packet.content().get(
        "info", new TypeToken<Collection<ProcessGroup>>() {});
    SyncAction action = packet.content().get("action", SyncAction.class);

    NodeExecutor.getInstance().getClusterSyncManager().handleProcessGroupSync(
        groups, action);
  }
}
