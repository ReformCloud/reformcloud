...
import systems.reformcloud.reformcloud2.executor.controller.network.packet.ClientPacketProcessWatchdogStopped;
            ...
            Streams.newList(ClientExecutor.getInstance().getProcessManager().getAll()).forEach(runningProcess -> {
                if (!runningProcess.isAlive()
                    ...
                    DefaultChannelManager.INSTANCE.get("Controller").ifPresent(packetSender -> packetSender.sendPacket(
                            new ClientPacketProcessWatchdogStopped(runningProcess.getProcessInformation().getProcessDetail().getName())
                    ));
                }
            });
