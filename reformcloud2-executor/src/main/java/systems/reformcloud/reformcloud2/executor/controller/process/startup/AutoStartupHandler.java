...
public final class AutoStartupHandler extends AbsoluteThread {
  ...
  public void update() {
    ...
    perPriorityStartup.addAll(ControllerExecutor.getInstance()
      ...
      Links.copySortedSet(perPriorityStartup).forEach(processGroup -> {
        int started =
            ControllerExecutor.getInstance()
                .getProcessManager()
                .getOnlineAndWaitingProcessCount(processGroup.getName());
        ...
        if (started <
          ...
          for (int i = started;
               ...
               processGroup.getStartupConfiguration().getMinOnlineProcesses();
               i++) {
            List<ProcessInformation> all = ControllerExecutor.getInstance()
                                               .getProcessManager()
                                               .getAllProcesses();
            if (ControllerExecutor.getInstance()
                ...
                ControllerExecutor.getInstance()
                        .getControllerConfig()
                        .getMaxProcesses() > all.size()) {
              if (processGroup.getStartupConfiguration()
                          .getMaxOnlineProcesses() == -1 ||
                  processGroup.getStartupConfiguration()
                          .getMaxOnlineProcesses() > i) {
                ControllerExecutor.getInstance()
                    .getProcessManager()
