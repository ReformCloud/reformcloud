...
public class DatabaseAPIImplementation
    implements DatabaseAsyncAPI, DatabaseSyncAPI {
  ...
  findAsync(@Nonnull String table, @Nonnull String key, String identifier) {
    ...
    Task.EXECUTOR.execute(() -> {
      ...
      if (result != null) {
        ...
        task.complete(
            databaseReader.findIfAbsent(identifier).getUninterruptedly());
      } else {
        task.complete(null);
      }
    });
  ...
  public boolean updateIfAbsent(@Nonnull String table,
                                ...
                                @Nonnull JsonConfiguration newData) {
    Boolean result =
        updateIfAbsentAsync(table, identifier, newData).getUninterruptedly();
    return result == null ? false : result;
  }
