...
public final class CommandReformCloud extends GlobalCommand {
  ...
  public boolean handleCommand(@Nonnull CommandSource commandSource,
    ...
    case "screen": {
      if (strings.length == 3) {
        ...
        if (processInformation.getNodeUniqueID().equals(
                NodeExecutor.getInstance().getNodeConfig().getUniqueID())) {
          NodeProcessScreenHandler
              .getScreen(processInformation.getProcessUniqueID())
              .ifPresent(
                  e
                  -> e.toggleFor(
          ...
          DefaultChannelManager.INSTANCE.get(processInformation.getParent())
              .ifPresent(
                  packetSender
                  -> packetSender.sendPacket(new NodePacketOutToggleScreen(
                      processInformation.getProcessUniqueID())));
        }
    ...
    case "copy": {
      ...
      if (NodeExecutor.getInstance().getNodeConfig().getUniqueID().equals(
              processInformation.getNodeUniqueID())) {
        Links
            .filterToReference(
    ...
    case "maintenance": {
      ...
      ExecutorAPI.getInstance()
          .getAsyncAPI()
          .getGroupAsyncAPI()
          .updateProcessGroupAsync(processGroup);
      System.out.println(LanguageManager.get("command-rc-execute-success"));
      return true;
    }
      ...
      if (strings.length == 4) {
        ...
        for (int started = 0; started <= i; started++) {
          ExecutorAPI.getInstance()
              .getAsyncAPI()
              .getProcessAsyncAPI()
              .startProcessAsync(processGroup.getName(), strings[3]);
          AbsoluteThread.sleep(TimeUnit.MILLISECONDS, 20);
        }
    ...
    case "stop": {
      if (strings.length == 2) {
        ...
        ExecutorAPI.getInstance()
            .getAsyncAPI()
            .getProcessAsyncAPI()
            .stopProcessAsync(processInformation.getProcessUniqueID());
        System.out.println(LanguageManager.get("command-rc-execute-success"));
        return true;
      }
      break;
    ...
    case "execute": {
      ...
      ExecutorAPI.getInstance()
          .getAsyncAPI()
          .getProcessAsyncAPI()
          .executeProcessCommandAsync(processInformation.getName(),
    ...
    case "create": {
      if (strings[1].equalsIgnoreCase("node") && strings.length == 4) {
        if (strings[2].split("\\.").length != 4) {
          System.out.println(
              LanguageManager.get("command-rc-node-ip-invalid", strings[2]));
          return true;
        }
        ...
        if (port == null || port < 0) {
          System.out.println(
              LanguageManager.get("command-rc-node-port-invalid", strings[3]));
          return true;
        }

        if (existsNode(strings[2], port)) {
          System.out.println(LanguageManager.get(
              "command-rc-node-already-exists", strings[2], strings[3]));
          return true;
        }
      ...
      if (strings[1].equalsIgnoreCase("main") && strings.length == 3) {
        ...
        if (mainGroup == null) {
          ExecutorAPI.getInstance()
              .getAsyncAPI()
              .getGroupAsyncAPI()
              .createMainGroupAsync(strings[2]);
          System.out.println(LanguageManager.get("command-rc-execute-success"));
        } else {
          System.out.println(LanguageManager.get(
        ...
        if (strings.length == 3) {
          ...
          if (processGroup == null) {
            ExecutorAPI.getInstance()
                .getAsyncAPI()
                .getGroupAsyncAPI()
                .createProcessGroupAsync(strings[2]);
            System.out.println(
            ...
            ExecutorAPI.getInstance()
                ...
                .createProcessGroupAsync(
                    ...
                    new PlayerAccessConfiguration(
                        ...
                        "reformcloud.join.maintenance", false, null, true, true,
                        true, 50),
                    staticProcess)
                .onComplete(e -> {
                  MainGroup mainGroup = ExecutorAPI.getInstance()
                ...
                .createProcessGroupAsync(
                    ...
                    Collections.singletonList(new Template(
                        ...
                        new RuntimeConfiguration(512, new ArrayList<>(),
                                                 new HashMap<>()),
                        version)),
                    new StartupConfiguration(max, min, 1, 41000,
                                             StartupEnvironment.JAVA_RUNTIME,
                                             true, new ArrayList<>()),
                    new PlayerAccessConfiguration(
                        "reformcloud.join.full", true,
      ...
      if (strings.length == 9) {
        ...
        if (version == null) {
          System.out.println(
              LanguageManager.get("command-rc-version-not-found", strings[3]));
          return true;
        }
      ...
      if (strings.length == 3) {
        if (strings[1].equalsIgnoreCase("sub")) {
          ...
          NodeExecutor.getInstance()
              ...
              .syncProcessGroupDelete(processGroup.getName());
          System.out.println(LanguageManager.get("command-rc-execute-success"));
          return true;
        }
        ...
        if (strings[1].equalsIgnoreCase("main")) {
          ...
          NodeExecutor.getInstance()
              .getClusterSyncManager()
              .syncMainGroupDelete(mainGroup.getName());
          System.out.println(LanguageManager.get("command-rc-execute-success"));
          return true;
        }
      }
  ...
  private boolean existsNode(String host, int port) {
    ...
    return Links
        .filterToReference(config.getOtherNodes(),
                           e
                           -> Links
                                  .deepFilterToReference(
                                      e,
                                      g
                                      -> g.getKey().equalsIgnoreCase(host) &&
                                             g.getValue() == port)
                                  .isPresent())
        .isPresent();
  }
