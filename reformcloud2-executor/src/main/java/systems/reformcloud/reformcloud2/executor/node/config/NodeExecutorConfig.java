...
public class NodeExecutorConfig {
  ...
  private static final Collection<Path> PATHS = newCollection(
      ...
      "reformcloud/configs", "reformcloud/applications", "reformcloud/temp",
      "reformcloud/static", "reformcloud/templates",
      "reformcloud/global/plugins", "reformcloud/files/.connection");
  ...
  private final Registry localMainGroupsRegistry =
      RegistryBuilder.newRegistry(Paths.get("reformcloud/groups/main"));

  private final Registry localSubGroupsRegistry =
  ...
  public void init() {
    ...
    if (!Files.exists(NodeConfig.PATH)) {
      ...
      setup
          .addQuestion(new DefaultSetupQuestion(
              ...
              e
              -> e.trim().split("\\.").length == 4,
              e
              -> new JsonConfiguration()
                     .add("config",
                          new NodeConfig(
                              CommonHelper.calculateMaxMemory(), e.trim(),
                              Collections.singletonList(
                                  Collections.singletonMap(e.trim(), 1809)),
                              Collections.singletonList(
                                  Collections.singletonMap(e.trim(), 2008)),
          ...
          .addQuestion(new DefaultSetupQuestion(
              ...
              s -> {
                ...
                this.localMainGroupsRegistry.createKey(mainProxy.getName(),
                                                       mainProxy);
                this.localMainGroupsRegistry.createKey(mainLobby.getName(),
                                                       mainLobby);
    ...
    this.nodeConfig =
        JsonConfiguration.read(NodeConfig.PATH).get("config", NodeConfig.TYPE);
    this.ingameMessages =
  ...
  private void loadGroups() {
    processGroups.addAll(this.localSubGroupsRegistry.readKeys(
        e -> e.get("key", ProcessGroup.TYPE)));
    mainGroups.addAll(this.localMainGroupsRegistry.readKeys(
  ...
  public NodeInformation getSelf() { return self; }
  ...
  public boolean isFirstStartup() { return firstStartup.get(); }
