...
public class ProcessAPIImplementation
    implements ProcessSyncAPI, ProcessAsyncAPI {
  ...
  startProcessAsync(@Nonnull String groupName, String template,
    ...
    Task.EXECUTOR.execute(() -> {
      ProcessGroup group =
          Links
              .filterToReference(NodeExecutor.getInstance()
                                     .getClusterSyncManager()
                                     .getProcessGroups(),
                                 e -> e.getName().equals(groupName))
              .orNothing();
      if (group == null) {
        task.complete(null);
        return;
      ...
      task.complete(nodeNetworkManager.startProcess(
          group,
          Links
              .filterToReference(group.getTemplates(),
                                 e -> e.getName().equals(template))
              .orNothing(),
          configurable));
    });
    return task;
  ...
  public Task<ProcessInformation> stopProcessAsync(@Nonnull String name) {
    Task<ProcessInformation> task = new DefaultTask<>();
    Task.EXECUTOR.execute(() -> {
      ProcessInformation old =
          nodeNetworkManager.getNodeProcessHelper().getClusterProcess(name);
      if (old == null) {
        task.complete(null);
        return;
      }
  ...
  public Task<ProcessInformation> stopProcessAsync(@Nonnull UUID uniqueID) {
    ...
    Task.EXECUTOR.execute(() -> {
      ProcessInformation old =
          nodeNetworkManager.getNodeProcessHelper().getClusterProcess(uniqueID);
      if (old == null) {
        task.complete(null);
        return;
      }
  ...
  public Task<ProcessInformation> getProcessAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(
                nodeNetworkManager.getNodeProcessHelper().getClusterProcess(
                    name)));
    return task;
  }
  ...
  getProcessesAsync(@Nonnull String group) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(Links.newList(
                nodeNetworkManager.getNodeProcessHelper().getClusterProcesses(
                    group))));
    return task;
  }
    ...
    Task.EXECUTOR.execute(() -> {
      ...
      if (NodeExecutor.getInstance().getNodeConfig().getUniqueID().equals(
        ...
        DefaultChannelManager.INSTANCE.get(processInformation.getParent())
            .ifPresent(e
                       -> e.sendPacket(new NodePacketOutExecuteCommand(
                           processInformation.getName(), commandLine)));
      }
  ...
  getGlobalOnlineCountAsync(@Nonnull Collection<String> ignoredProxies) {
    ...
    Task.EXECUTOR.execute(() -> {
      int online = Links
                       .allOf(nodeNetworkManager.getNodeProcessHelper()
                                  .getClusterProcesses(),
                              e
                              -> !e.getTemplate().isServer() &&
                                     !ignoredProxies.contains(e.getName()))
                       .stream()
  ...
  public List<ProcessInformation> getProcesses(@Nonnull String group) {
    List<ProcessInformation> information =
        getProcessesAsync(group).getUninterruptedly();
  ...
  public int getGlobalOnlineCount(@Nonnull Collection<String> ignoredProxies) {
    Integer integer =
        getGlobalOnlineCountAsync(ignoredProxies).getUninterruptedly();
