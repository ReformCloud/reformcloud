...
public class NodeProcessScreen {
  ...
  public NodeProcessScreen(UUID uniqueID) { this.uniqueID = uniqueID; }
  ...
  public void toggleFor(String name) {
    ...
    if (name.equals(NodeExecutor.getInstance().getNodeConfig().getName())) {
      ...
      DefaultChannelManager.INSTANCE.get(name).ifPresent(
          e -> e.sendPacket(new NodePacketOutScreenEnabled(uniqueID, queue)));
    }
  }
  ...
  void addScreenLine(String line) {
    ...
    enabledFrom.stream()
        ...
        .forEach(
            e
            -> e.sendPacket(new NodePacketOutScreenLineAdded(uniqueID, line)));

    ProcessInformation processInformation =
        ExecutorAPI.getInstance().getSyncAPI().getProcessSyncAPI().getProcess(
            uniqueID);
    if (processInformation != null &&
        enabledFrom.contains(
