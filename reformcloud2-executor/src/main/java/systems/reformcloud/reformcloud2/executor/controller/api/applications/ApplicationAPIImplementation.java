...
public class ApplicationAPIImplementation
    implements ApplicationAsyncAPI, ApplicationSyncAPI {

  public ApplicationAPIImplementation(
      @Nonnull ApplicationLoader applicationLoader) {
  ...
  loadApplicationAsync(@Nonnull InstallableApplication application) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(
                applicationLoader.doSpecificApplicationInstall(application)));
    return task;
  }
  ...
  unloadApplicationAsync(@Nonnull LoadedApplication application) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(
                applicationLoader.doSpecificApplicationUninstall(application)));
    return task;
  }
