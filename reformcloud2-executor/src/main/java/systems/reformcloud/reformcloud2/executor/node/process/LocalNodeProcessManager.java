...
public class LocalNodeProcessManager implements NodeProcessManager {
  ...
  public ProcessInformation getLocalCloudProcess(String name) {
    return Links
        .filterToReference(
            information,
            e -> e.getName().equals(name) && isLocal(e.getProcessUniqueID()))
        .orNothing();
  }
  ...
  public ProcessInformation getLocalCloudProcess(UUID uuid) {
    return Links
        .filterToReference(information,
                           e
                           -> e.getProcessUniqueID().equals(uuid) &&
                                  isLocal(e.getProcessUniqueID()))
        .orNothing();
  }

  @Override
  public ProcessInformation
  startLocalProcess(ProcessGroup processGroup, Template template,
                    JsonConfiguration data, UUID processUniqueID) {
    int id = nextID(processGroup);
    ProcessInformation processInformation = new ProcessInformation(
        processGroup.getName() + template.getServerNameSplitter() + id,
        processGroup.getName() + (processGroup.isShowIdInName()
                                      ? (template.getServerNameSplitter() + id)
                                      : ""),
        NodeExecutor.getInstance().getNodeConfig().getName(),
        NodeExecutor.getInstance().getNodeConfig().getUniqueID(),
        processUniqueID, id, ProcessState.PREPARED,
        new NetworkInfo(
            NodeExecutor.getInstance().getNodeConfig().getStartHost(),
            nextPort(processGroup.getStartupConfiguration().getStartPort()),
            false),
        processGroup, template, ProcessRuntimeInformation.empty(),
        new ArrayList<>(), data,
        processGroup.getPlayerAccessConfiguration().getMaxPlayers());
    NodeInformation information = NodeExecutor.getInstance()
                                      .getNodeNetworkManager()
                                      .getCluster()
                                      .getSelfNode();
    information.addUsedMemory(processInformation.getTemplate()
  ...
  public ProcessInformation stopLocalProcess(String name) {
    List<LocalNodeProcess> processes =
  ...
  public ProcessInformation stopLocalProcess(UUID uuid) {
    List<LocalNodeProcess> processes =
        LocalProcessManager.getNodeProcesses()
            ...
            .filter(
                e
                -> e.getProcessInformation().getProcessUniqueID().equals(uuid))
            .collect(Collectors.toList());
    if (processes.isEmpty()) {
      return null;
    }
  ...
  @Override
  public ProcessInformation
  queueProcess(ProcessGroup processGroup, Template template,
               JsonConfiguration data, NodeInformation node, UUID uniqueID) {
    ProcessInformation processInformation =
        constructCaInfo(processGroup, template, data, node, uniqueID);
    DefaultChannelManager.INSTANCE.get(node.getName())
        .ifPresent(e
                   -> e.sendPacket(new NodePacketOutQueueProcess(
  ...
  public void unregisterLocalProcess(UUID uniqueID) {
    Links
        .filterToReference(information,
                           e -> e.getProcessUniqueID().equals(uniqueID))
  ...
  public void handleLocalProcessStart(ProcessInformation processInformation) {
    ...
    information.getStartedProcesses().add(new NodeProcess(
        ...
        processInformation.getName(), processInformation.getProcessUniqueID()));
    NodeExecutor.getInstance().getClusterSyncManager().syncSelfInformation();
  }
  ...
  public void handleLocalProcessStop(ProcessInformation processInformation) {
    ...
    NodeInformation information = NodeExecutor.getInstance()
                                      ...
                                      .getCluster()
                                      .getSelfNode();
    information.removeUsedMemory(processInformation.getTemplate()
                                     .getRuntimeConfiguration()
                                     .getMaxMemory());
    Links
        .filterToReference(
            ...
            e
            -> e.getUniqueID().equals(processInformation.getProcessUniqueID()))
  ...
  public void handleProcessStart(ProcessInformation processInformation) {
    ProcessInformation information =
        Links
            .filterToReference(this.information,
                               e
  ...
  public void handleProcessConnection(ProcessInformation processInformation) {
    Task.EXECUTOR.execute(() -> {
      while (!DefaultChannelManager.INSTANCE.get(processInformation.getName())
                  .isPresent()) {
      ...
      DefaultChannelManager.INSTANCE.get(processInformation.getName())
          .ifPresent(e
                     -> e.sendPacket(new ControllerEventProcessUpdated(
                         processInformation)));
    });
  }
    ...
    return Links
        .filterToReference(
            ...
            -> e.getName().equals(name) &&
                   e.getNodeUniqueID().equals(NodeExecutor.getInstance()
                                                  .getNodeNetworkManager()
                                                  .getCluster()
                                                  .getSelfNode()
                                                  .getNodeUniqueID()))
  ...
  public boolean isLocal(UUID uniqueID) {
    return Links
        .filterToReference(
            information,
            e
            -> e.getProcessUniqueID().equals(uniqueID) &&
                   e.getNodeUniqueID().equals(NodeExecutor.getInstance()
                                                  .getNodeNetworkManager()
                                                  .getCluster()
                                                  .getSelfNode()
  ...
  public Collection<ProcessInformation> getClusterProcesses(String group) {
    return Links.allOf(information,
                       e -> e.getProcessGroup().getName().equals(group));
  }
  ...
  public void update(@Nonnull ProcessInformation processInformation) {
    ...
    NodeExecutor.getInstance().getClusterSyncManager().syncProcessUpdate(
        processInformation);
  }

  private void removeProcess(ProcessInformation information) {
    NodeProcess nodeProcess = Links.filter(
        NodeExecutor.getInstance()
            .getNodeNetworkManager()
            .getCluster()
            .getSelfNode()
            .getStartedProcesses(),
        e -> e.getUniqueID().equals(information.getProcessUniqueID()));
    if (nodeProcess == null) {
      return;
    }
  ...
  private int nextPort(int startPort) {
    Collection<Integer> ports = information.stream()
                                    .map(e -> e.getNetworkInfo().getPort())
                                    .collect(Collectors.toList());
  ...
  constructCaInfo(ProcessGroup processGroup, Template template,
                  JsonConfiguration data, NodeInformation node, UUID uniqueID) {
    int id = nextID(processGroup);
    return new ProcessInformation(
        ...
        processGroup.getName() + (processGroup.isShowIdInName()
                                      ? (template.getServerNameSplitter() + id)
                                      : ""),
        node.getName(), node.getNodeUniqueID(), uniqueID, id,
        ProcessState.PREPARED, new NetworkInfo(null, -1, false), processGroup,
        template, ProcessRuntimeInformation.empty(), new ArrayList<>(), data,
        processGroup.getPlayerAccessConfiguration().getMaxPlayers());
  }
}
