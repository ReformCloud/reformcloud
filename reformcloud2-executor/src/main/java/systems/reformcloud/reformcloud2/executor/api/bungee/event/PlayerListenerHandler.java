...
public final class PlayerListenerHandler implements Listener {
  ...
  public void handle(final ServerConnectEvent event) {
    ...
    if (proxiedPlayer.getServer() == null) {
      ProcessInformation lobby = BungeeExecutor.getBestLobbyForPlayer(
          BungeeExecutor.getInstance().getThisProcessInformation(),
  ...
  public void handle(final ServerConnectedEvent event) {
    DefaultChannelManager.INSTANCE.get("Controller")
        .ifPresent(
            sender
            -> sender.sendPacket(new APIBungeePacketOutPlayerServerSwitch(
  ...
  public void handle(final LoginEvent event) {
    ...
    if (ExecutorAPI.getInstance()
      ...
      sender.sendPacket(new APIPacketOutCreateLoginRequest(
          connection.getUniqueId(), connection.getName()));
    }
  }
  ...
  public void handle(final PostLoginEvent event) {
    ...
    if (configuration.isJoinOnlyPerPermission() &&
      ...
      player.disconnect(TextComponent.fromLegacyText(
          format(BungeeExecutor.getInstance()
                     .getMessages()
    ...
    BungeeExecutor.getInstance().setThisProcessInformation(
  ...
  public void handle(final PlayerDisconnectEvent event) {
    ...
    CommonHelper.EXECUTOR.execute(() -> {
      ProcessInformation current = ExecutorAPI.getInstance()
                                       ...
                                       .getProcessSyncAPI()
                                       .getThisProcessInformation();
      if (ProxyServer.getInstance().getOnlineCount() <
