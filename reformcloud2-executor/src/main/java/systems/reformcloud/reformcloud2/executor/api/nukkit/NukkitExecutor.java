...
public final class NukkitExecutor extends API implements PlayerAPIExecutor {
  ...
  NukkitExecutor(Plugin plugin) {
    ...
    packetHandler.registerHandler(
        new APIPacketInPluginAction(new PluginsExecutorContainer()));
  ...
  public void executeSendMessage(UUID player, String message) {
    Server.getInstance().getPlayer(player).ifPresent(
        player1 -> player1.sendMessage(message));
  }

  @Override
  public void executeKickPlayer(UUID player, String message) {
    Server.getInstance().getPlayer(player).ifPresent(
        player1 -> player1.kick(message));
  }
  ...
  public void executeSendTitle(UUID player, String title, String subTitle,
    ...
    Server.getInstance().getPlayer(player).ifPresent(
        player1 -> player1.sendTitle(title, subTitle, fadeIn, stay, fadeOut));
  }
  ...
  public void executeTeleport(UUID player, String world, double x, double y,
    ...
    Server.getInstance().getPlayer(player).ifPresent(
        player1 -> player1.teleport(new Location(x, y, z, yaw, pitch)));
  }
