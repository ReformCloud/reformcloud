...
public class LocalProcessQueue extends AbsoluteThread {
  ...
  private static final BlockingDeque<LocalNodeProcess> QUEUE =
  ...
  public static void queue(ProcessInformation processInformation) {
    LocalNodeProcess localNodeProcess =
        new BasicLocalNodeProcess(processInformation);
    localNodeProcess.prepare();
    int size = QUEUE.size();
    QUEUE.offerLast(localNodeProcess);
    System.out.println(LanguageManager.get(
        "client-process-now-in-queue", processInformation.getName(), size + 1));
  }
  ...
  public void run() {
    while (!Thread.currentThread().isInterrupted()) {
      ...
      try {
        ...
        if (isMemoryFree(process.getProcessInformation()
                             .getTemplate()
                             .getRuntimeConfiguration()
                             .getMaxMemory()) &&
            process.bootstrap()) {
          System.out.println(LanguageManager.get(
              "node-process-start", process.getProcessInformation().getName()));
  ...
  private boolean isMemoryFree(int memory) {
    return NodeExecutor.getInstance().getNodeConfig().getMaxMemory() >=
        (NodeExecutor.getInstance()
             ...
             .stream()
             .mapToInt(
                 e -> e.getTemplate().getRuntimeConfiguration().getMaxMemory())
             .sum() +
         memory);
  }
}
