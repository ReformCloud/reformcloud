...
public final class VelocityExecutor extends API implements PlayerAPIExecutor {
  ...
  private static final List<ProcessInformation> LOBBY_SERVERS =
      new ArrayList<>();
  ...
  VelocityExecutor(VelocityLauncher launcher, ProxyServer proxyServer) {
    ...
    proxyServer.getEventManager().register(launcher,
  ...
  public void handleProcessRemove(ProcessInformation processInformation) {
    proxyServer.getServer(processInformation.getName())
        .ifPresent(
            registeredServer
  ...
  getBestLobbyForPlayer(ProcessInformation current, Player player,
  ...
  public void executeSendMessage(UUID player, String message) {
    proxyServer.getPlayer(player).ifPresent(
        player1 -> player1.sendMessage(TextComponent.of(message)));
  }

  @Override
  public void executeKickPlayer(UUID player, String message) {
    proxyServer.getPlayer(player).ifPresent(
        player1 -> player1.disconnect(TextComponent.of(message)));
  }
  ...
  public void executeConnect(UUID player, String server) {
    proxyServer.getPlayer(player).ifPresent(player1 -> {
      if (isServerRegistered(server)) {
        player1.createConnectionRequest(proxyServer.getServer(server).get())
            .fireAndForget();
      }
    });
  }
  ...
  public void executeConnect(UUID player, UUID target) {
    proxyServer.getPlayer(player).ifPresent(
        player1 -> proxyServer.getPlayer(target).ifPresent(targetPlayer -> {
          if (targetPlayer.getCurrentServer().isPresent() &&
              isServerRegistered(targetPlayer.getCurrentServer()
                                     .get()
                                     .getServerInfo()
                                     .getName())) {
            player1
  ...
  public void executeSetResourcePack(UUID player, String pack) {
    proxyServer.getPlayer(player).ifPresent(
        player1 -> player1.sendResourcePack(pack));
  }
}
