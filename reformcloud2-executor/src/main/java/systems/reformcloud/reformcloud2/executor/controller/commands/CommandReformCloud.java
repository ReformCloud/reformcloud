...
public final class CommandReformCloud extends GlobalCommand {
  ...
  public CommandReformCloud() {
    super("rc", "reformcloud.command.rc",
          "The main management command for reformcloud",
          Arrays.asList("reformcloud", "servers", "process", "proxies"));
  }
  ...
  public boolean handleCommand(@Nonnull CommandSource commandSource,
    ...
    if (strings.length == 1 && strings[0].equalsIgnoreCase("versions")) {
      {
        System.out.println(
            LanguageManager.get("command-rc-available-versions", "Java-Proxy"));
        StringBuilder stringBuilder = new StringBuilder();
        Version.getJavaProxyProviders().forEach(
            (s, v) -> stringBuilder.append(v.name()).append(", "));
        System.out.println(
            stringBuilder.substring(0, stringBuilder.length() - 2));
      }

      {
        System.out.println(LanguageManager.get("command-rc-available-versions",
                                               "Pocket-Edition-Proxy"));
        StringBuilder stringBuilder = new StringBuilder();
        Version.getPocketProxyProviders().forEach(
            (s, v) -> stringBuilder.append(v.name()).append(", "));
        System.out.println(
            stringBuilder.substring(0, stringBuilder.length() - 2));
      }

      {
        System.out.println(LanguageManager.get("command-rc-available-versions",
                                               "Java-Server"));
        StringBuilder stringBuilder = new StringBuilder();
        Version.getJavaServerProviders().forEach(
            (s, v) -> stringBuilder.append(v.name()).append(", "));
        System.out.println(
            stringBuilder.substring(0, stringBuilder.length() - 2));
      }
      ...
      {
        System.out.println(LanguageManager.get("command-rc-available-versions",
                                               "Pocket-Edition-Server"));
        StringBuilder stringBuilder = new StringBuilder();
        Version.getPocketServerProviders().forEach(
            (s, v) -> stringBuilder.append(v.name()).append(", "));
        System.out.println(
            stringBuilder.substring(0, stringBuilder.length() - 2));
      }
      ...
      ClientManager.INSTANCE.getClientRuntimeInformation().forEach(
    ...
    case "screen": {
      if (strings.length == 3) {
        ...
        if (uuid == null) {
          processInformation = ExecutorAPI.getInstance()
                                   .getSyncAPI()
                                   .getProcessSyncAPI()
                                   .getProcess(strings[1]);
        } else {
          processInformation = ExecutorAPI.getInstance()
                                   .getSyncAPI()
                                   .getProcessSyncAPI()
        ...
        if (processInformation == null) {
          System.out.println(
              LanguageManager.get("command-rc-process-unknown", strings[1]));
          return true;
        }
        ...
        DefaultChannelManager.INSTANCE.get(processInformation.getParent())
            .ifPresent(
                packetSender
                -> packetSender.sendPacket(new ControllerPacketOutToggleScreen(
                    processInformation.getProcessUniqueID())));
    ...
    case "copy": {
      ...
      DefaultChannelManager.INSTANCE.get(processInformation.getParent())
          .ifPresent(
              packetSender
              -> packetSender.sendPacket(new ControllerPacketOutCopyProcess(
                  processInformation.getProcessUniqueID())));
      System.out.println(LanguageManager.get("command-rc-execute-success"));
      return true;
    }
    ...
    case "maintenance": {
      ...
      if (processGroup == null) {
        System.out.println(
            LanguageManager.get("command-rc-group-unknown", strings[1]));
        return true;
      }
    ...
    case "start": {
      if (strings.length == 2 &&
          strings[1].equalsIgnoreCase("internalclient")) {
        if (!Files.exists(Paths.get("reformcloud/.client"))) {
          System.out.println(
              LanguageManager.get("command-rc-internal-client-not-installed"));
          return true;
        }
        ...
        if (ClientManager.INSTANCE.getProcess() == null) {
          ...
          System.out.println(LanguageManager.get(
              "command-rc-internal-client-already-started"));
        }
        return true;
      }
      ...
      if (strings.length == 3) {
        ...
        if (i == null || i < 1) {
          System.out.println(
              LanguageManager.get("command-rc-integer-failed", strings[2]));
          return true;
        }
      ...
      if (strings.length == 4) {
        ...
        for (int started = 0; started <= i; started++) {
          ExecutorAPI.getInstance()
              .getSyncAPI()
              .getProcessSyncAPI()
              .startProcess(processGroup.getName(), strings[3]);
          AbsoluteThread.sleep(TimeUnit.MILLISECONDS, 20);
        }
    ...
    case "stop": {
      if (strings.length == 2 &&
        ...
        if (ClientManager.INSTANCE.getProcess() != null) {
          ...
          System.out.println(
              LanguageManager.get("command-rc-internal-client-not-started"));
        }
        return true;
      }
    ...
    case "ofall": {
      if (strings.length == 3) {
        if (strings[2].equalsIgnoreCase("list")) {
          ...
          if (mainGroup == null) {
            System.out.println(LanguageManager.get(
                "command-rc-main-group-unknown", strings[1]));
            return true;
          }

          mainGroup.getSubGroups().forEach(
              s
              -> System.out.println(
                  LanguageManager.get("command-rc-main-sub-group", s)));
          return true;
    ...
    case "execute": {
      ...
      ExecutorAPI.getInstance()
          .getSyncAPI()
          .getProcessSyncAPI()
          .executeProcessCommand(processInformation.getName(),
    ...
    case "create": {
      if (strings.length == 3 &&
        ...
        if (Files.exists(Paths.get("reformcloud/.client"))) {
          System.out.println(
              LanguageManager.get("command-rc-internal-client-already-exists"));
          return true;
        }

        if (strings[2].split("\\.").length != 4) {
          System.out.println(
              LanguageManager.get("command-rc-expected-ip", strings[2]));
          return true;
        }

        if (ControllerExecutor.getInstance()
                .getControllerConfig()
                .getNetworkListener()
                .size() == 0) {
          System.out.println(
              LanguageManager.get("command-rc-no-network-listener-configured"));
          return true;
        }

        SystemHelper.createDirectory(
            Paths.get("reformcloud/.client/reformcloud/.bin"));
        SystemHelper.createDirectory(
            Paths.get("reformcloud/.client/reformcloud/files/.connection"));
        SystemHelper.doCopy(
            "reformcloud/.bin/config.properties",
            "reformcloud/.client/reformcloud/.bin/config.properties");
        SystemHelper.doCopy(
            "reformcloud/.bin/connection.json",
            "reformcloud/.client/reformcloud/files/.connection/connection.json");
        CommonHelper.rewriteProperties(
            "reformcloud/.client/reformcloud/.bin/config.properties",
            "ReformCloudController edit", (UnaryOperator<String>)s -> {
        ...
        new JsonConfiguration()
            .add("config", new ClientConfig(CommonHelper.calculateMaxMemory(),
                                            -1, 90.0D, strings[2]))
        ...
        Map<String, Integer> map =
            ControllerExecutor.getInstance()
                .getControllerConfig()
                .getNetworkListener()
                .get(new Random().nextInt(ControllerExecutor.getInstance()
                                              .getControllerConfig()
                                              .getNetworkListener()
                                              .size()));
        new JsonConfiguration()
      ...
      if (strings[1].equalsIgnoreCase("main") && strings.length == 3) {
        MainGroup mainGroup = ExecutorAPI.getInstance()
                                  .getSyncAPI()
                                  .getGroupSyncAPI()
                                  .getMainGroup(strings[2]);
        if (mainGroup == null) {
          ExecutorAPI.getInstance()
              .getSyncAPI()
              .getGroupSyncAPI()
              .createMainGroup(strings[2]);
          System.out.println(LanguageManager.get("command-rc-execute-success"));
        } else {
          System.out.println(LanguageManager.get(
              "command-rc-create-main-group-already-exists", strings[2]));
        }
        return true;
      }
      ...
      if (strings[1].equalsIgnoreCase("sub")) {
        if (strings.length == 3) {
          ProcessGroup processGroup = ExecutorAPI.getInstance()
                                          .getSyncAPI()
                                          .getGroupSyncAPI()
                                          .getProcessGroup(strings[2]);
          if (processGroup == null) {
            ExecutorAPI.getInstance()
                .getSyncAPI()
                .getGroupSyncAPI()
                .createProcessGroup(strings[2]);
            System.out.println(
                LanguageManager.get("command-rc-execute-success"));
          } else {
            System.out.println(LanguageManager.get(
                "command-rc-create-sub-group-already-exists", strings[2]));
          }
          return true;
        }
        ...
        if (strings.length == 4) {
          ProcessGroup processGroup = ExecutorAPI.getInstance()
                                          ...
                                          .getProcessGroup(strings[2]);
          Version version =
              CommonHelper.findEnumField(Version.class, strings[3]).orNothing();
          if (version == null) {
            System.out.println(LanguageManager.get(
                "command-rc-version-not-found", strings[3]));
            return true;
          }
            ...
            ExecutorAPI.getInstance()
                ...
                .createProcessGroup(
                    ...
                    Collections.singletonList(new Template(
                        ...
                        new RuntimeConfiguration(512, new ArrayList<>(),
                                                 new HashMap<>()),
                        version)));
        ...
        if (strings.length == 6) {
          ...
          if (staticProcess == null) {
            System.out.println(
                LanguageManager.get("command-rc-required-boolean", strings[5]));
            return true;
          }
                ...
                ExecutorAPI.getInstance()
                    ...
                    .createProcessGroup(
                        ...
                        new StartupConfiguration(
                            -1, 1, 1, 41000, StartupEnvironment.JAVA_RUNTIME,
                            true, new ArrayList<>()),
                        new PlayerAccessConfiguration(
                            "reformcloud.join.full", true,
                            "reformcloud.join.maintenance", false, null, true,
                            true, true, 50),
                        staticProcess);
        ...
        if (strings.length == 7) {
          ...
          if (lobby == null) {
            System.out.println(
                LanguageManager.get("command-rc-required-boolean", strings[6]));
            return true;
          }
          ...
          if (processGroup == null) {
            ProcessGroup processGroup1 =
                new DefaultProcessGroup(strings[2], 41000, version, 512, false,
                                        50, staticProcess, lobby);
            ...
            ExecutorAPI.getInstance()
                .getSyncAPI()
                .getGroupSyncAPI()
                .createProcessGroup(processGroup1);
            System.out.println(
                LanguageManager.get("command-rc-execute-success"));
          } else {
            System.out.println(LanguageManager.get(
        ...
        if (strings.length == 8) {
          ...
          if (max == null || max < -1) {
            System.out.println(
                LanguageManager.get("command-rc-integer-failed", strings[7]));
            return true;
          }

          if (min == null || min < 0) {
            System.out.println(
                LanguageManager.get("command-rc-integer-failed", strings[6]));
            return true;
          }
                ...
                ExecutorAPI.getInstance()
                    ...
                    .createProcessGroup(
                        ...
                        Collections.singletonList(new Template(
                            ...
                            version)),
                        new StartupConfiguration(
                            max, min, 1, 41000, StartupEnvironment.JAVA_RUNTIME,
                            true, new ArrayList<>()),
                        new PlayerAccessConfiguration(
                            "reformcloud.join.full", true,
                            "reformcloud.join.maintenance", false, null, true,
        ...
        if (strings.length == 9) {
          ...
          if (max == null || max < -1) {
            System.out.println(
                LanguageManager.get("command-rc-integer-failed", strings[8]));
            return true;
          }
          ...
          if (processGroup == null) {
            ProcessGroup processGroup1 =
                new DefaultProcessGroup(strings[2], 41000, version, 512, false,
                                        min, max, staticProcess, lobby);

            ExecutorAPI.getInstance()
                .getAsyncAPI()
                .getGroupAsyncAPI()
                .createProcessGroupAsync(processGroup1)
                .onComplete(e -> {
                  MainGroup mainGroup = ExecutorAPI.getInstance()
                                            .getSyncAPI()
                                            .getGroupSyncAPI()
      ...
      if (strings.length == 2 && strings[1].equalsIgnoreCase("sub")) {
        ExecutorAPI.getInstance()
            ...
            .forEach(processGroup
                     -> System.out.println(
                         "  => " + processGroup.getName() + " maintenance: " +
                         processGroup.getPlayerAccessConfiguration()
                             .isMaintenance() +
    ...
    case "delete": {
      if (strings.length == 3) {
        if (strings[1].equalsIgnoreCase("sub")) {
          ...
          if (processGroup == null) {
            System.out.println(
                LanguageManager.get("command-rc-group-unknown", strings[2]));
            return true;
          }

          ControllerExecutor.getInstance()
              .getControllerExecutorConfig()
              .deleteProcessGroup(processGroup);
          System.out.println(LanguageManager.get("command-rc-execute-success"));
          return true;
        }
        ...
        if (strings[1].equalsIgnoreCase("main")) {
          ...
          if (mainGroup == null) {
            System.out.println(LanguageManager.get(
                "command-rc-main-group-unknown", strings[2]));
            return true;
          }
