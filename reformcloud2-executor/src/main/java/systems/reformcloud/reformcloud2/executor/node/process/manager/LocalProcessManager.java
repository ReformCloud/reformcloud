...
public class LocalProcessManager {
  ...
  private static final Collection<LocalNodeProcess> NODE_PROCESSES =
      new ArrayList<>();
  ...
  public static void unregisterProcesses(UUID uniqueID) {
    Links
        .filterToReference(
            NODE_PROCESSES,
            e
            -> e.getProcessInformation().getProcessUniqueID().equals(uniqueID))
        .ifPresent(NODE_PROCESSES::remove);
  }
