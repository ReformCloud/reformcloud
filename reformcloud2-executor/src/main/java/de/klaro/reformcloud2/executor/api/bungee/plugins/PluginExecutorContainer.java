...
public final class PluginExecutorContainer implements PluginExecutor {
    ...
    static {
        try {
            Method method = net.md_5.bungee.api.plugin.Plugin.class.getDeclaredMethod("init", ProxyServer.class, PluginDescription.class);
    ...
    private void loadPlugin(InstallablePlugin plugin) {
        ...
        try (JarFile jarFile = new JarFile(pluginFile)) {
            ...
            try (InputStream inputStream = jarFile.getInputStream(descriptionFile)) {
                ...
                net.md_5.bungee.api.plugin.Plugin actual = (net.md_5.bungee.api.plugin.Plugin) mainClass.getDeclaredConstructor().newInstance();

                //Init plugin
                init.invoke(actual, ProxyServer.getInstance(), description);

                //Add plugin
                Map<String, net.md_5.bungee.api.plugin.Plugin> map = (Map<String, net.md_5.bungee.api.plugin.Plugin>) plugins.get(ProxyServer.getInstance().getPluginManager());
    ...
    private void disablePlugin(net.md_5.bungee.api.plugin.Plugin plugin) {
        if (plugin == null) {
            return;
        }
        ...
        try {
            Map<String, net.md_5.bungee.api.plugin.Plugin> pluginMap = (Map<String, net.md_5.bungee.api.plugin.Plugin>) plugins.get(pluginManager);
            pluginMap.values().remove(plugin);

            Multimap<net.md_5.bungee.api.plugin.Plugin, Command> commandMultimap = (Multimap<net.md_5.bungee.api.plugin.Plugin, Command>) commands.get(pluginManager);
            commandMultimap.removeAll(plugin);

            Multimap<net.md_5.bungee.api.plugin.Plugin, Listener> listenerMultimap = (Multimap<net.md_5.bungee.api.plugin.Plugin, Listener>) listeners.get(pluginManager);
