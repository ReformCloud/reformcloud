...
public abstract class ExternalAPIImplementation extends ExecutorAPI
    implements ProcessSyncAPI, ProcessAsyncAPI, ApplicationSyncAPI,
               ApplicationAsyncAPI, ClientSyncAPI, ClientAsyncAPI,
               ConsoleSyncAPI, ConsoleAsyncAPI, DatabaseSyncAPI,
               DatabaseAsyncAPI, GroupSyncAPI, GroupAsyncAPI, PlayerSyncAPI,
  ...
  @Override
  public Task<Boolean>
  loadApplicationAsync(@Nonnull InstallableApplication application) {
    Task<Boolean> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutLoadApplication(application),
                packet
  ...
  @Override
  public Task<Boolean>
  unloadApplicationAsync(@Nonnull LoadedApplication application) {
  ...
  public Task<Boolean> unloadApplicationAsync(@Nonnull String application) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutUnloadApplication(application),
                packet
  ...
  public Task<LoadedApplication> getApplicationAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetLoadedApplication(name),
                packet
                -> task.complete(
                    packet.content().get("result", LoadedApplication.TYPE))));
    return task;
  }
  ...
  public Task<List<LoadedApplication>> getApplicationsAsync() {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetApplications(),
                packet
                -> task.complete(new ArrayList<>(packet.content().get(
                    "result",
                    new TypeToken<List<DefaultLoadedApplication>>() {})))));
    return task;
  }
  ...
  public Task<Boolean> isClientConnectedAsync(String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(getClientInformationAsync(name).getUninterruptedly(
                                 TimeUnit.SECONDS, 5) != null));
    return task;
  }
  ...
  public Task<String> getClientStartHostAsync(String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(getClientInformationAsync(name)
  ...
  public Task<ClientRuntimeInformation> getClientInformationAsync(String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetClientRuntimeInformation(name),
                packet
                -> task.complete(packet.content().get(
                    "result", ClientRuntimeInformation.TYPE))));
    return task;
  }
  ...
  @Override
  public Task<String>
  dispatchCommandAndGetResultAsync(@Nonnull String commandLine) {
    Task<String> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDispatchControllerCommand(commandLine),
                packet -> task.complete(packet.content().getString("result"))));
    return task;
  }
  ...
  public Task<Command> getCommandAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutGetCommand(name),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", Command.TYPE))));
    return task;
  ...
  public Task<Boolean> isCommandRegisteredAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(getCommandAsync(name).getUninterruptedly(
                                 TimeUnit.SECONDS, 5) != null));
    return task;
  }
  ...
  @Nonnull
  @Override
  public Task<JsonConfiguration>
  findAsync(@Nonnull String table, @Nonnull String key, String identifier) {
    Task<JsonConfiguration> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDatabaseFindDocument(table, key,
                                                             identifier),
                packet -> task.complete(packet.content().get("result"))));
    return task;
  }
  ...
  findAsync(@Nonnull String table, @Nonnull String key, String identifier,
    ...
    Task.EXECUTOR.execute(
        ()
            -> task.complete(function.apply(
                findAsync(table, key, identifier).getUninterruptedly())));
    return task;
  }

  @Nonnull
  @Override
  public Task<Void> insertAsync(@Nonnull String table, @Nonnull String key,
                                String identifier,
                                @Nonnull JsonConfiguration data) {
    Task<Void> task = new DefaultTask<>();
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutDatabaseInsertDocument(
  ...
  @Nonnull
  @Override
  public Task<Boolean> updateAsync(@Nonnull String table, @Nonnull String key,
                                   @Nonnull JsonConfiguration newData) {
    Task<Boolean> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDatabaseUpdateDocument(table, key,
                                                               newData, true),
                packet
  ...
  @Nonnull
  @Override
  public Task<Boolean> updateIfAbsentAsync(@Nonnull String table,
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDatabaseUpdateDocument(
                    table, identifier, newData, false),
                packet
                -> task.complete(packet.content().getBoolean("result"))));
    return task;
  }
  ...
  public Task<Void> removeAsync(@Nonnull String table, @Nonnull String key) {
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(
          new ExternalAPIPacketOutDatabaseRemoveDocument(table, key, true));
      task.complete(null);
    });
    return task;
  }
  ...
  @Nonnull
  @Override
  public Task<Void> removeIfAbsentAsync(@Nonnull String table,
                                        @Nonnull String identifier) {
    Task<Void> task = new DefaultTask<>();
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutDatabaseRemoveDocument(
          table, identifier, false));
      task.complete(null);
    });
    return task;
  }
  ...
  public Task<Boolean> createDatabaseAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDatabaseAction(
                    ExternalAPIPacketOutDatabaseAction.DatabaseAction.CREATE,
                    name),
                packet
                -> task.complete(packet.content().getBoolean("result"))));
    return task;
  ...
  public Task<Boolean> deleteDatabaseAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDatabaseAction(
                    ExternalAPIPacketOutDatabaseAction.DatabaseAction.DELETE,
                    name),
                packet
                -> task.complete(packet.content().getBoolean("result"))));
    return task;
  ...
  @Nonnull
  @Override
  public Task<Boolean> containsAsync(@Nonnull String table,
                                     @Nonnull String key) {
    Task<Boolean> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDatabaseContainsDocument(table, key),
                packet
                -> task.complete(packet.content().getBoolean("result"))));
    return task;
  }
  ...
  public Task<Integer> sizeAsync(@Nonnull String table) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutDatabaseAction(
                    ExternalAPIPacketOutDatabaseAction.DatabaseAction.SIZE,
                    table),
                packet
  ...
  @Override
  public JsonConfiguration find(@Nonnull String table, @Nonnull String key,
                                String identifier) {
    return findAsync(table, key, identifier).getUninterruptedly();
  }

  @Override
  public <T> T find(@Nonnull String table, @Nonnull String key,
                    String identifier,
                    @Nonnull Function<JsonConfiguration, T> function) {
  ...
  @Override
  public void insert(@Nonnull String table, @Nonnull String key,
                     String identifier, @Nonnull JsonConfiguration data) {
    insertAsync(table, key, identifier, data).awaitUninterruptedly();
  }

  @Override
  public boolean update(@Nonnull String table, @Nonnull String key,
                        @Nonnull JsonConfiguration newData) {
    return updateAsync(table, key, newData).getUninterruptedly();
  }

  @Override
  public boolean updateIfAbsent(@Nonnull String table,
                                @Nonnull String identifier,
                                @Nonnull JsonConfiguration newData) {
  ...
  @Override
  public void removeIfAbsent(@Nonnull String table,
                             @Nonnull String identifier) {
  ...
  public Task<MainGroup> createMainGroupAsync(@Nonnull String name,
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutCreateMainGroup(
                                   new MainGroup(name, subgroups)),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", MainGroup.TYPE))));
    return task;
  }
  ...
  createProcessGroupAsync(@Nonnull ProcessGroup processGroup) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutCreateProcessGroup(processGroup),
                packet
                -> task.complete(
                    packet.content().get("result", ProcessGroup.TYPE))));
    return task;
  }
  ...
  public Task<MainGroup> getMainGroupAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutGetMainGroup(name),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", MainGroup.TYPE))));
    return task;
  ...
  public Task<ProcessGroup> getProcessGroupAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutGetProcessGroup(name),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", ProcessGroup.TYPE))));
    return task;
  }
  ...
  public Task<List<MainGroup>> getMainGroupsAsync() {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetMainGroups(),
                packet
                -> task.complete(packet.content().get(
                    "result", new TypeToken<List<MainGroup>>() {}))));
    return task;
  }
  ...
  public Task<List<ProcessGroup>> getProcessGroupsAsync() {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetProcessGroups(),
                packet
                -> task.complete(packet.content().get(
                    "result", new TypeToken<List<ProcessGroup>>() {}))));
    return task;
  }
  ...
  public Task<Void> sendMessageAsync(@Nonnull UUID player,
    ...
    Task<Void> task = new DefaultTask<>();
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.SEND_MESSAGE, player,
          message));
      task.complete(null);
    });
    return task;
  ...
  public Task<Void> kickPlayerAsync(@Nonnull UUID player,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.KICK_PLAYER, player,
          message));
      task.complete(null);
    });
    return task;
  ...
  public Task<Void> kickPlayerFromServerAsync(@Nonnull UUID player,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.KICK_SERVER, player,
          message));
      task.complete(null);
    });
    return task;
  ...
  public Task<Void> playSoundAsync(@Nonnull UUID player, @Nonnull String sound,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.PLAY_SOUND, player, sound, f1,
          f2));
      task.complete(null);
    });
    return task;
  ...
  public Task<Void> sendTitleAsync(@Nonnull UUID player, @Nonnull String title,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.SEND_TITLE, player, title,
  ...
  public Task<Void> playEffectAsync(@Nonnull UUID player,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.PLAY_ENTITY_EFFECT, player,
          entityEffect));
      task.complete(null);
    });
    return task;
  ...
  public <T> Task<Void> playEffectAsync(@Nonnull UUID player,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.PLAY_EFFECT, player, effect,
          data));
      task.complete(null);
    });
    return task;
  ...
  public Task<Void> respawnAsync(@Nonnull UUID player) {
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.RESPAWN, player));
      task.complete(null);
    });
    return task;
  }
  ...
  public Task<Void> teleportAsync(@Nonnull UUID player, @Nonnull String world,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.LOCATION_TELEPORT, player,
  ...
  public Task<Void> connectAsync(@Nonnull UUID player, @Nonnull String server) {
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.CONNECT, player, server));
      task.complete(null);
    });
    return task;
  }
  ...
  public Task<Void> connectAsync(@Nonnull UUID player, @Nonnull UUID target) {
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.CONNECT_PLAYER, player,
          target));
      task.complete(null);
    });
    return task;
  ...
  public Task<Void> setResourcePackAsync(@Nonnull UUID player,
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(new ExternalAPIPacketOutAPIAction(
          ExternalAPIPacketOutAPIAction.APIAction.SET_RESOURCE_PACK, player,
          pack));
      task.complete(null);
    });
    return task;
  ...
  public Task<Plugin> getInstalledPluginAsync(@Nonnull String process,
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetInstalledPlugin(name, process),
                packet
  ...
  getPluginsAsync(@Nonnull String process, @Nonnull String author) {
    ...
    Task.EXECUTOR.execute(()
                              -> task.complete(Links.allOf(
                                  getPlugins(process),
                                  plugin -> plugin.author().equals(author))));
    return task;
  }
  ...
  getPluginsAsync(@Nonnull String process) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetPlugins(process),
                packet
                -> task.complete(packet.content().get(
                    "result", new TypeToken<Collection<DefaultPlugin>>() {}))));
    return task;
  }
  ...
  @Nonnull
  @Override
  public Task<ProcessInformation>
  startProcessAsync(@Nonnull String groupName, String template,
                    @Nonnull JsonConfiguration configurable) {
    Task<ProcessInformation> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutStartProcess(
                                   groupName, template, configurable),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", ProcessInformation.TYPE))));
    return task;
  }
  ...
  public Task<ProcessInformation> stopProcessAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutStopProcess(name),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", ProcessInformation.TYPE))));
    return task;
  ...
  public Task<ProcessInformation> stopProcessAsync(@Nonnull UUID uniqueID) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutStopProcess(uniqueID),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", ProcessInformation.TYPE))));
    return task;
  ...
  public Task<ProcessInformation> getProcessAsync(@Nonnull String name) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutGetProcess(name),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", ProcessInformation.TYPE))));
    return task;
  ...
  public Task<ProcessInformation> getProcessAsync(@Nonnull UUID uniqueID) {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(new ExternalAPIPacketOutGetProcess(uniqueID),
                               packet
                               -> task.complete(packet.content().get(
                                   "result", ProcessInformation.TYPE))));
    return task;
  ...
  public Task<List<ProcessInformation>> getAllProcessesAsync() {
    ...
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetProcesses(),
                packet
                -> task.complete(packet.content().get(
                    "result", new TypeToken<List<ProcessInformation>>() {}))));
    return task;
  }

  @Nonnull
  @Override
  public Task<List<ProcessInformation>>
  getProcessesAsync(@Nonnull String group) {
    Task<List<ProcessInformation>> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetProcesses(group),
                packet
                -> task.complete(packet.content().get(
                    "result", new TypeToken<List<ProcessInformation>>() {}))));
    return task;
  ...
  @Nonnull
  @Override
  public Task<Void> executeProcessCommandAsync(@Nonnull String name,
                                               @Nonnull String commandLine) {
    Task<Void> task = new DefaultTask<>();
    Task.EXECUTOR.execute(() -> {
      sendPacket(
          new ExternalAPIPacketOutExecuteProcessCommand(name, commandLine));
      task.complete(null);
    });
    return task;
  }
  ...
  @Override
  public Task<Integer>
  getGlobalOnlineCountAsync(@Nonnull Collection<String> ignoredProxies) {
    Task<Integer> task = new DefaultTask<>();
    Task.EXECUTOR.execute(
        ()
            -> sendPacketQuery(
                new ExternalAPIPacketOutGetOnlineCount(ignoredProxies),
                packet
                -> task.complete(packet.content().getInteger("result"))));
    return task;
  }
  ...
  updateAsync(@Nonnull ProcessInformation processInformation) {
    ...
    Task.EXECUTOR.execute(() -> {
      sendPacket(
          new ExternalAPIPacketOutUpdateProcessInformation(processInformation));
      task.complete(null);
    });
    return task;
  }
  ...
  startProcess(@Nonnull String groupName, String template,
               @Nonnull JsonConfiguration configurable) {
    return startProcessAsync(groupName, template, configurable)
        .getUninterruptedly();
  }
  ...
  @Override
  public void executeProcessCommand(@Nonnull String name,
                                    @Nonnull String commandLine) {
  ...
  private void sendPacket(Packet packet) {
    DefaultChannelManager.INSTANCE.get("Controller")
        .ifPresent(packetSender -> packetSender.sendPacket(packet));
  }
  ...
  private void sendPacketQuery(Packet packet, Consumer<Packet> result) {
    DefaultChannelManager.INSTANCE.get("Controller")
        .ifPresent(packetSender
                   -> packetHandler()
                          .getQueryHandler()
                          .sendQueryAsync(packetSender, packet)
                          .getTask()
                          .onFailure(e -> result.accept(null))
                          .onComplete(result));
  }
}
