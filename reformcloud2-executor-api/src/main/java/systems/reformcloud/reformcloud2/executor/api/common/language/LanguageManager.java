...
public final class LanguageManager {
  ...
  private static final Map<LanguageSource, Language> languagePerSource =
      new HashMap<>();
  ...
  public static String getOrDefault(String key, String def,
    ...
    if (message == null) {
      List<Language> languages = languagePerAddon.values()
                                     .stream()
                                     .filter(e -> e.messages().contains(key))
                                     .collect(Collectors.toList());
      if (languages.isEmpty()) {
        message = def;
      } else {
        message =
            languages.stream().findAny().get().messages().getProperty(key, def);
      }
    }
