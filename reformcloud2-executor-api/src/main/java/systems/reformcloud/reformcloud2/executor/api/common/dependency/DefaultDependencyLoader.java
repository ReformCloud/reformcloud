...
public final class DefaultDependencyLoader extends DependencyLoader {
  ...
  static {
    ...
    try (InputStream inputStream =
             DefaultDependencyLoader.class.getClassLoader().getResourceAsStream(
                 "internal/versions.properties")) {
  ...
  private static final List<Dependency> DEFAULT_DEPENDENCIES = Arrays.asList(
      new DefaultDependency(DefaultRepositories.MAVEN_CENTRAL, "io.netty",
                            "netty-all", PROPERTIES),
      new DefaultDependency(DefaultRepositories.MAVEN_CENTRAL, "jline", "jline",
                            PROPERTIES),
      new DefaultDependency(DefaultRepositories.MAVEN_CENTRAL,
                            "com.google.code.gson", "gson", PROPERTIES),
      new DefaultDependency(DefaultRepositories.MAVEN_CENTRAL, "org.yaml",
                            "snakeyaml", PROPERTIES),
      new DefaultDependency(DefaultRepositories.MAVEN_CENTRAL,
                            "org.reflections", "reflections", PROPERTIES),
      new DefaultDependency(DefaultRepositories.MAVEN_CENTRAL,
                            "com.google.guava", "guava", PROPERTIES),
      new DefaultDependency(DefaultRepositories.MAVEN_CENTRAL, "org.javassist",
                            "javassist", PROPERTIES));
  ...
  public void loadDependencies() {
    DEFAULT_DEPENDENCIES.forEach(dependency -> {
      System.out.println("Preloading dependency " + dependency.getArtifactID() +
  ...
  public URL loadDependency(@Nonnull Dependency dependency) {
    ...
    if (DefaultDependencyLoader.path != null) {
      path =
          Paths.get(DefaultDependencyLoader.path + "/" + dependency.getPath());
    } else {
