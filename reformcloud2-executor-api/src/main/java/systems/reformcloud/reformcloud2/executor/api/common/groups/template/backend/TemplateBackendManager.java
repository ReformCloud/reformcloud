...
public class TemplateBackendManager {
  ...
  private static final Collection<TemplateBackend> DEFAULTS =
      Collections.singletonList(new FileBackend());

  public static TemplateBackend getOrDefault(String name) {
    TemplateBackend backend =
        Links
            .filterToReference(BACKENDS,
  ...
  public static void registerBackend(TemplateBackend templateBackend) {
    TemplateBackend backend =
        Links
            .filterToReference(
                BACKENDS,
                e -> e.getName().equalsIgnoreCase(templateBackend.getName()))
            .orNothing();
    if (backend == null) {
  ...
  public static void unregisterBackend(String name) {
    Links.filterToReference(BACKENDS, e -> e.getName().equalsIgnoreCase(name))
        .ifPresent(BACKENDS::remove);
  }
