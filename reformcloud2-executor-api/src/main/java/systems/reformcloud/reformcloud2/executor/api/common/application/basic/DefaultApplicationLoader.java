...
public final class DefaultApplicationLoader implements ApplicationLoader {
  ...
  private static final DependencyLoader DEPENDENCY_LOADER =
      new DefaultDependencyLoader();

  private final List<ApplicationHandler> applicationHandlers =
      new ArrayList<>();
  ...
  public void detectApplications() {
    ...
    for (File file : Objects.requireNonNull(APP_DIR.listFiles(
             pathname
      ...
      try (JarFile jarFile = new JarFile(file)) {
        ...
        Conditions.isTrue(
            appConfig != null,
            "Application has to contain a 'application.json' or 'app.json'");
        try (InputStream inputStream = jarFile.getInputStream(appConfig)) {
          ...
          ApplicationConfig applicationConfig =
              new ApplicationConfigBuilder(configurable.getString("name"),
                                           configurable.getString("main"),
                                           configurable.getString("author"),
                                           configurable.getString("version"),
                                           file, appConfig)
                  .withDependencies(configurable.getOrDefault(
                      "dependencies",
          ...
          Conditions.isTrue(applicationConfig.getName() != null,
                            "Application has no name");
          Conditions.isTrue(applicationConfig.main() != null,
                            "Application must have a main class");
          Conditions.isTrue(applicationConfig.author() != null,
                            "Application must have a author");
          Conditions.isTrue(applicationConfig.version() != null,
                            "Application must have a version");
          Conditions.isTrue(applicationConfig.implementedVersion() != null,
                            "Application must have an api-version");
      ...
      try {
        if (config.getValue().dependencies().length != 0) {
          for (Dependency dependency : config.getValue().dependencies()) {
            DEPENDENCY_LOADER.addDependency(
                DEPENDENCY_LOADER.loadDependency(dependency));
          }
        }

        AppClassLoader classLoader = new AppClassLoader(
            new URL[] {config.getValue().applicationFile().toURI().toURL()},
            Thread.currentThread().getContextClassLoader());

        Class<?> main = classLoader.loadClass(config.getValue().main());
        Conditions.isTrue(main != null, "Main-Class of application " +
                                            config.getKey() + " not found");
        Application application =
            (Application)main.getDeclaredConstructor().newInstance();

        System.out.println(LanguageManager.get("successfully-pre-installed-app",
                                               config.getKey(),
                                               config.getValue().author()));
        application.onInstallable();

        application.init(
            new DefaultLoadedApplication(this, config.getValue(), main),
            classLoader);
        application.onInstalled();
        application.getApplication().setApplicationStatus(
            ApplicationStatus.INSTALLED);
        System.out.println(
            LanguageManager.get("successfully-installed-app", config.getKey()));
  ...
  public void loadApplications() {
    applications.forEach(application -> {
      ...
      System.out.println(LanguageManager.get(
          "successfully-loaded-app", application.getApplication().getName()));
    });
  ...
  public void enableApplications() {
    applications.forEach(application -> {
      ...
      System.out.println(LanguageManager.get(
          "successfully-enabled-app", application.getApplication().getName()));
    });
  ...
  public void disableApplications() {
    applications.forEach(application -> {
      application.getApplication().setApplicationStatus(
          ApplicationStatus.PRE_DISABLE);
      application.onPreDisable();
      System.out.println(
          LanguageManager.get("successfully-pre-disabled-app",
      ...
      System.out.println(
          LanguageManager.get("successfully-uninstalled-app",
                              application.getApplication().getName()));
      application.getApplication().setApplicationStatus(
  ...
  doSpecificApplicationInstall(@Nonnull InstallableApplication application) {
    DownloadHelper.downloadAndDisconnect(application.url(),
                                         "reformcloud/applications/" +
                                             application.getName() + ".jar");
    File file =
        new File("reformcloud/applications/" + application.getName() + ".jar");
    if (!file.exists()) {
      return false;
    }
      ...
      try (InputStream inputStream = jarFile.getInputStream(appConfig)) {
        ...
        ApplicationConfig applicationConfig =
            new ApplicationConfigBuilder(
                configurable.getString("name"), configurable.getString("main"),
                configurable.getString("author"),
                configurable.getString("version"), file, appConfig)
                .withDependencies(configurable.get(
                    "dependencies", new TypeToken<List<Dependency>>() {}))
                .withDescription(configurable.getString("description"))
                .withWebsite(configurable.getString("website"))
                .withImplementedVersion(configurable.getString("impl-version"))
                .create();
        ...
        AppClassLoader classLoader = new AppClassLoader(
            new URL[] {applicationConfig.applicationFile().toURI().toURL()},
            Thread.currentThread().getContextClassLoader());
        ...
        System.out.println(LanguageManager.get("successfully-installed-app",
                                               applicationConfig.getName()));
  ...
  doSpecificApplicationUninstall(@Nonnull LoadedApplication loadedApplication) {
    Application application =
        Links.filter(applications,
                     application1
                     -> application1.getApplication().getName().equals(
  ...
  public LoadedApplication getApplication(@Nonnull String name) {
    return Links.filterAndApply(applications,
                                app
                                -> app.getApplication().getName().equals(name),
  ...
  @Override
  public String
  getApplicationName(@Nonnull LoadedApplication loadedApplication) {
  ...
  public List<LoadedApplication> getApplications() {
    return Collections.unmodifiableList(
        Links.apply(applications, Application::getApplication));
  }

  @Override
  public void
  addApplicationHandler(@Nonnull ApplicationHandler applicationHandler) {
