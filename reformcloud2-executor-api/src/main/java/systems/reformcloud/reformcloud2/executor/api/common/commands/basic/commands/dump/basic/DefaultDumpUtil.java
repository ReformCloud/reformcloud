...
public class DefaultDumpUtil implements DumpUtil {
  ...
  private void bumpNetwork(StringBuilder stringBuilder) {
    List<PacketSender> allSender =
        DefaultChannelManager.INSTANCE.getAllSender();
    stringBuilder.append("--- Connected Channels (")
        .append(allSender.size())
        .append(") ---");
    ...
    if (allSender.size() > 0) {
      allSender.forEach(e -> {
        long connectionTime =
            System.currentTimeMillis() - e.getConnectionTime();
  ...
  private void bumpMainGroups(StringBuilder stringBuilder) {
    List<MainGroup> mainGroups = ExecutorAPI.getInstance()
                                     .getSyncAPI()
                                     .getGroupSyncAPI()
                                     .getMainGroups();
    stringBuilder.append("--- Registered Main Groups (")
        .append(mainGroups.size())
        .append(") ---");
    stringBuilder.append("\n");
    ...
    if (mainGroups.size() > 0) {
      mainGroups.forEach(
          ...
          -> stringBuilder.append("Name: ")
                 ...
                 .append("\n")
                 .append("SubGroups: ")
                 .append(String.format("%d total, %s", g.getSubGroups().size(),
                                       String.join(" ", g.getSubGroups())))
                 .append("\n\n"));
    } else {
  ...
  private void bumpProcessGroups(StringBuilder stringBuilder) {
    List<ProcessGroup> processGroups = ExecutorAPI.getInstance()
                                           .getSyncAPI()
                                           .getGroupSyncAPI()
                                           .getProcessGroups();
    stringBuilder.append("--- Registered Sub Groups (")
        .append(processGroups.size())
        .append(") ---");
    stringBuilder.append("\n");
    ...
    if (processGroups.size() > 0) {
      processGroups.forEach(e -> {
            ...
            e.getTemplates()
                ...
                .map(t -> {
                  ...
                  return "Name: " + t.getName() +
                      ...
                      "\n   Global: " + t.isGlobal() +
                      "\n   System-Properties: " +
                      String.format("%d total, %s", env.size(),
                                    String.join(" ", env)) +
                      "\n   Parameters: " +
                      String.format(
                          "%d total, %s",
                          t.getRuntimeConfiguration()
                              ...
                              .size(),
                          String.join(" ", t.getRuntimeConfiguration()
                                               .getProcessParameters()));
                })
                .collect(Collectors.toList());
        ...
        stringBuilder.append("Name: ")
            ...
            .append("\n")
            .append("Templates: ")
            .append(String.format("%d total, %s", e.getTemplates().size(),
  ...
  private void bumpStartedProcesses(StringBuilder stringBuilder) {
    List<ProcessInformation> allProcesses = ExecutorAPI.getInstance()
                                                .getSyncAPI()
                                                .getProcessSyncAPI()
                                                .getAllProcesses();
    stringBuilder.append("--- Started Processes (")
        .append(allProcesses.size())
        .append(") ---");
    stringBuilder.append("\n");

    if (allProcesses.size() > 0) {
      allProcesses.forEach(e -> {
        Collection<String> players = e.getOnlinePlayers()
                                         .stream()
                                         .map(Player::getName)
                                         .collect(Collectors.toList());
        ...
        stringBuilder.append("Name: ")
            ...
            .append("\n")
            .append("Online: ")
            .append(String.format("%d total, %s", players.size(),
                                  String.join(" ", players)))
            .append("\n")
            .append("Plugins: ")
            .append(String.format("%d total, %s", plugins.size(),
                                  String.join(" ", plugins)))
            .append("\n")
            .append("Template: ")
            .append("\n")
  ...
  private void bumpLoadedApplications(StringBuilder stringBuilder) {
    List<LoadedApplication> applications = ExecutorAPI.getInstance()
                                               .getSyncAPI()
                                               .getApplicationSyncAPI()
                                               .getApplications();
    stringBuilder.append("--- Loaded Applications (")
