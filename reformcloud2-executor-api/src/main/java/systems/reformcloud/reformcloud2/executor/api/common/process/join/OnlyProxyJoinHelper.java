...
public final class OnlyProxyJoinHelper {
  ...
  private OnlyProxyJoinHelper() { throw new UnsupportedOperationException(); }
  ...
  public static boolean walkedOverProxy(UUID uuid, String name) {
    JoinRequest joinRequest =
        Links.filter(JOIN_REQUESTS,
                     joinRequest1
                     -> joinRequest1.getUniqueID().equals(uuid) &&
                            joinRequest1.getName().equals(name));
    if (joinRequest == null) {
      return false;
    }
  ...
  public static void onDisconnect(UUID uuid) {
    JoinRequest joinRequest = Links.filter(
        JOIN_REQUESTS, joinRequest1 -> joinRequest1.getUniqueID().equals(uuid));
    if (joinRequest != null) {
