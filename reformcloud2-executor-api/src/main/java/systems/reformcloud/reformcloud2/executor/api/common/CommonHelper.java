...
public final class CommonHelper {
  ...
  private CommonHelper() { throw new UnsupportedOperationException(); }
  ...
  public static void rewriteProperties(String path, String saveComment,
    ...
    try (InputStream inputStream = Files.newInputStream(Paths.get(path))) {
      ...
      properties.stringPropertyNames().forEach(
          s -> properties.setProperty(s, function.apply(s)));

      try (OutputStreamWriter outputStreamWriter =
               new OutputStreamWriter(Files.newOutputStream(Paths.get(path)),
  ...
  public static int calculateMaxMemory() {
    return (
        int)((operatingSystemMXBean().getTotalPhysicalMemorySize() / 1048576) -
             2048);
  }
  ...
  public static <T extends Enum<T>> ReferencedOptional<T>
  findEnumField(Class<T> enumClass, String field) {
    Map<String, WeakReference<? extends Enum<?>>> cached =
        CACHE.computeIfAbsent(enumClass, aClass -> cache(enumClass));

    WeakReference<? extends Enum<?>> reference = cached.get(field);
    return reference == null
        ? ReferencedOptional.empty()
        : ReferencedOptional.build(enumClass.cast(reference.get()));
  }

  private static <T extends Enum<T>>
      Map<String, WeakReference<? extends Enum<?>>> cache(Class<T> enumClass) {
