...
public final class DefaultRestAPIHandler extends RestAPIHandler {
  ...
  private void handleHttpRequest(ChannelHandlerContext channelHandlerContext,
                                 String path, HttpRequest httpRequest) {
    HttpHeaders httpHeaders = httpRequest.headers();
    if (!httpHeaders.contains("-XUser") || !httpHeaders.contains("-XToken")) {
      channelHandlerContext.channel()
          .writeAndFlush(new DefaultFullHttpResponse(
              httpRequest.protocolVersion(), HttpResponseStatus.UNAUTHORIZED))
          .addListener(CLOSE);
      return;
    }

    Configurable<JsonConfiguration> configurable =
        new JsonConfiguration()
            .add("name", httpHeaders.get("-XUser"))
            .add("token", httpHeaders.get("-XToken"));
    Double<Boolean, WebRequester> auth =
        tryAuth(channelHandlerContext, configurable);
    if (!auth.getFirst()) {
      channelHandlerContext.channel()
          .writeAndFlush(new DefaultFullHttpResponse(
              httpRequest.protocolVersion(), HttpResponseStatus.UNAUTHORIZED))
    ...
    Links
        .allOf(requestHandler.getHandlers(),
               e -> e.path().equals(path) && e.canAccess(auth.getSecond()))
        .forEach(
            e
            -> e.handleRequest(auth.getSecond(), httpRequest, httpResponse -> {
    ...
    channelHandlerContext.channel().closeFuture().addListener(
        (ChannelFutureListener)
            channelFuture -> CompletableFuture.runAsync(() -> {
  ...
  private Double<Boolean, WebRequester>
  tryAuth(ChannelHandlerContext channelHandlerContext,
          Configurable<JsonConfiguration> configurable) {
    return requestHandler.authHandler().handleAuth(configurable,
                                                   channelHandlerContext);
  }
}
