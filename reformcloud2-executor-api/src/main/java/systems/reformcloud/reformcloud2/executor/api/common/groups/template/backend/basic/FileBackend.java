...
public class FileBackend implements TemplateBackend {
  ...
  public void createTemplate(String group, String template) {
    if (!existsTemplate(group, template)) {
      SystemHelper.createDirectory(
          Paths.get("reformcloud/templates", group, template, "plugins"));
    }
  }
  ...
  public void loadGlobalTemplates(ProcessGroup group, Path target) {
    Links.allOf(group.getTemplates(), Template::isGlobal)
        .forEach(e -> loadTemplate(group.getName(), e.getName(), target));
  }
  ...
  public void deployTemplate(String group, String template, Path current) {
    ...
    SystemHelper.copyDirectory(current, format(group, template),
                               Arrays.asList("log-out.log", "runner.jar"));
  }
