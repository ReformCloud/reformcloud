...
public final class DefaultEventManager implements EventManager {
  ...
  private final Map<Class<?>, List<LoadedListener>> done =
  ...
  public void unregisterAll() {
    Links.forEachValues(
        ...
        -> Links.forEach(
            loadedListeners,
            loadedListener -> unregister(loadedListener.getListener())));
  }
  ...
  private Map<Class<?>, Map<Byte, Set<Method>>> find(Object listener) {
    ...
    for (Method method : listener.getClass().getDeclaredMethods()) {
      ...
      if (annotation != null) {
        ...
        Conditions.isTrue(
            parameters.length == 1,
            "Listener class {0} tried to register a method with {1} instead of one argument",
            listener.getClass().getSimpleName(), parameters.length);

        Map<Byte, Set<Method>> map =
            result.computeIfAbsent(parameters[0], aClass -> new HashMap<>());

        Set<Method> methods = map.computeIfAbsent(
            annotation.priority().getPriority(), aByte -> new HashSet<>());
  ...
  private void register(Object listener) {
    ...
    lock.lock();
    try {
      for (Map.Entry<Class<?>, Map<Byte, Set<Method>>> classMapEntry :
           handlers.entrySet()) {
        Map<Byte, Map<Object, Method[]>> priorities =
            byListenerAndPriority.computeIfAbsent(classMapEntry.getKey(),
                                                  aClass -> new HashMap<>());

        for (Map.Entry<Byte, Set<Method>> byteSetEntry :
             classMapEntry.getValue().entrySet()) {
          Map<Object, Method[]> current = priorities.computeIfAbsent(
              byteSetEntry.getKey(), aByte -> new HashMap<>());
  ...
  private void finallyDo(Class<?> eventClass) {
    Map<Byte, Map<Object, Method[]>> map =
          ...
          for (Map.Entry<Object, Method[]> objectEntry : handlers.entrySet()) {
            for (Method method : objectEntry.getValue()) {
              LoadedListener loadedListener =
                  new LoadedListener(objectEntry.getKey(), method);
    ...
    try {
      for (Map.Entry<Class<?>, Map<Byte, Set<Method>>> classMapEntry :
        ...
        Map<Byte, Map<Object, Method[]>> prioritiesMap =
            byListenerAndPriority.get(classMapEntry.getKey());
