...
// Note! This class CANNOT use Reflections because it leads to problems using
// spigot (older guava implementation)
public final class ExternalEventBusHandler {

  /**
   * Initializes the external event bus which handles all known event packets
   * and call them sync
   *
   ...
   */
  public ExternalEventBusHandler(@Nonnull PacketHandler packetHandler,
                                 @Nonnull EventManager eventManager) {
    packetHandler.registerNetworkHandlers(
        new EventPacketInProcessClosed(), new EventPacketInProcessStarted(),
        new EventPacketInProcessUpdated(),
        new EventPacketInPlayerServerSwitch(), new EventPacketInLogoutPlayer(),
   ...
   */
  public void callEvent(@Nonnull Event event) { eventManager.callEvent(event); }
}
