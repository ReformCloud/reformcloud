...
public final class DefaultCommandManager implements CommandManager {
  ...
  private final Map<Command, String> noPermissionMessagePerCommand =
      new HashMap<>();
  ...
  public Command unregisterAndGetCommand(@Nonnull String line) {
    ...
    for (Command command : Links.newList(commands)) {
      if (command.mainCommand().equals(line) ||
          command.aliases().contains(line)) {
  ...
  @Override
  public Command dispatchCommandEvent(@Nonnull CommandEvent commandEvent,
  ...
  @Override
  public void register(@Nonnull String noPermissionMessage,
                       @Nonnull Command command) {
    dispatchCommandEvent(CommandEvent.ADD, command);
  ...
  @Override
  public void dispatchCommand(@Nonnull CommandSource commandSource,
    ...
    if (command.permission() != null &&
        !commandSource.hasPermission(command.permission())) {
      String noPermMessage = this.noPermissionMessagePerCommand.getOrDefault(
          command, NO_PERMISSIONS);
      result.accept(noPermMessage);
      return;
    }

    if (!command.sources().equals(AllowedCommandSources.ALL) &&
        !command.sources().equals(commandSources)) {
      result.accept(LanguageManager.get("command-source-not-allowed",
                                        commandSources.name()));
      return;
    }

    String[] strings = split.length == 1
                           ? new String[0]
                           : Arrays.copyOfRange(split, 1, split.length);
    if (!command.handleCommand(commandSource, strings)) {
      new Exception("Error in handling command:: status=false")
          .printStackTrace();
