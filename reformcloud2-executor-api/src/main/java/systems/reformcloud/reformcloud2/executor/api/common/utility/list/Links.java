...
public final class Links {
   ...
   * @return A new list with all values of the incoming list, applied to the
   *     function
   */
  @Nonnull
  public static <T, F> List<F> apply(@Nonnull List<T> in,
                                     @Nonnull Function<T, F> function) {
   ...
   * @param predicate The predicate which checks if the current object equals
   *     the the filter
   * @param <T> The object parameter of the values in the list
   * @return The first value in the list which equals to the filter
   */
  @Nullable
  public static <T> T filter(@Nonnull Collection<T> in,
   ...
   *     the the filter
   * @param <T> The object parameter of the values in the list
   * @return A new {@link ReferencedOptional} with the value or {@code null} if
   *     no value in the list equals to the filter
   */
  @Nonnull
  public static <T> ReferencedOptional<T>
  filterToReference(@Nonnull Collection<T> in,
                    @Nonnull Predicate<T> predicate) {
   ...
   * @return A new {@link ReferencedOptional} with the value or {@code null} if
   *     no value in the map equals to the filter
   */
  @Nonnull
  public static <K, V> ReferencedOptional<V>
  filterToReference(@Nonnull Map<K, V> in, @Nonnull Predicate<K> predicate) {
    if (in.isEmpty()) {
  ...
  /**
   * Filters a value out of the given list and applies the function if the value
   * is non-{@code null}
   *
   * @see #filter(Collection, Predicate)
   ...
   * @param predicate The predicate which checks if the current object equals
   *     the the filter
   * @param function The function which should get applied to the first value
   *     which equals to the filter
   * @param <T> The object parameter of the values in the list
   * @param <F> The object parameter of the outgoing value
   * @return The value which got applied to the function or {@code null} if no
   *     value in the list equals to the filter
   */
  @Nullable
  public static <T, F> F filterAndApply(@Nonnull List<T> in,
                                        @Nonnull Predicate<T> predicate,
                                        @Nonnull Function<T, F> function) {
    if (in.isEmpty()) {
      return null;
   ...
   */
  @Nonnull
  public static <F, T> List<T> getValues(@Nonnull Map<F, T> in,
                                         @Nonnull Predicate<F> predicate) {
    if (in.isEmpty()) {
   ...
   * @param <T> The object parameter of the values in the map
   */
  public static <T> void forEachValues(@Nonnull Map<?, T> map,
   ...
   */
  public static <F> void forEach(@Nonnull List<F> list,
                                 @Nonnull Consumer<F> consumer) {
   ...
   *
   * @param map The given map
   * @param fxFunction The function which should get applied to all keys in the
   *     map
   * @param <F> The object parameter of the keys in the map
   ...
   */
  @Nonnull
  public static <F, X> List<X> keyApply(@Nonnull Map<F, ?> map,
                                        @Nonnull Function<F, X> fxFunction) {
  ...
  @Nonnull
  @SafeVarargs
  public static <S, F> Collection<F>
  newCollection(@Nonnull Function<S, F> function, S... in) {
    return newCollection(Arrays.asList(in), function);
  }

  /**
   * Applies all values of the incoming collection to the function and collect
   * the values
   *
  ...
  @Nonnull
  public static <S, F> Collection<F>
  newCollection(@Nonnull Collection<S> in, @Nonnull Function<S, F> function) {
   ...
   *
   * @param in The incoming collection
   * @param predicate The {@link Predicate} which checks if the value of the
   *     list should get into the new one
   * @param function The function which maps the value of the incoming
   *     collection to the outgoing one
  ...
  @Nonnull
  public static <S, F> Collection<F>
  newCollection(@Nonnull Collection<S> in, @Nonnull Predicate<S> predicate,
                @Nonnull Function<S, F> function) {
    return in.stream().filter(predicate).map(function).collect(
        Collectors.toList());
  }

  /**
   * Filters all values out of the incoming collection which matches to the
   * {@link Predicate}
   *
   ...
   */
  @Nonnull
  public static <T> List<T> list(@Nonnull Collection<T> list,
                                 @Nonnull Predicate<T> predicate) {
    return list.stream().filter(predicate).collect(Collectors.toList());
  }

  /**
   * Filters all values out of the incoming collection which not matches to the
   * {@link Predicate}
   *
   * @param list The incoming list
   * @param predicate The predicate which tests the values in the list
   ...
   */
  @Nonnull
  public static <T> Collection<T> others(@Nonnull Collection<T> list,
                                         @Nonnull Predicate<T> predicate) {
    return list.stream()
   ...
   */
  @Nonnull
  public static <T> Collection<T> allOf(Collection<T> collection,
   ...
   */
  @Nonnull
  public static <T, F> Collection<F> apply(@Nonnull Collection<T> collection,
                                           @Nonnull Function<T, F> function) {
   ...
   */
  @Nonnull
  public static <F, S> Collection<S>
  deepFilter(@Nonnull Map<F, S> in,
             @Nonnull Predicate<Map.Entry<F, S>> predicate) {
   ...
   * @param <F> The object parameter of the keys in the map
   * @param <S> The object parameter of the values in the map
   * @return The first {@link Map.Entry} which matches to filter or {@link
   *     ReferencedOptional#empty()}
   */
  @Nonnull
  public static <F, S> ReferencedOptional<Map.Entry<F, S>>
  deepFilterToReference(@Nonnull Map<F, S> in,
                        @Nonnull Predicate<Map.Entry<F, S>> predicate) {
    if (in.isEmpty()) {
      return ReferencedOptional.empty();
    }
